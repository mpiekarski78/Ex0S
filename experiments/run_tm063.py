...
TM.0.6.2 kept bind=push through never-wipe train, then a C life on that dirty S
committed p98 and did not stamp TUNE. Genome sat on HOLD at an unnamed station.
This slice: if S already has notes but none name here, do not sit on the species
prior, and on success stamp an unmarked rare page (commit one if needed).
...
from experiments.run_tm061 import clutter_prose, wiki_prose
from experiments.run_tm062 import (
    _w_flags as _w_flags062,
    classify_a as _classify_a062,
    classify_b as _classify_b062,
    classify_common as _classify_common062,
    make as _make062,
    run_arm as _run_arm062,
)
...
def _run_dir() -> Path:
    ...
    d = REPO_ROOT / "runs" / f"{stamp}_tm063"
    d.mkdir(parents=True, exist_ok=True)
    return d


def make(*args, use_stamp_new_here: bool = True, **kwargs):
    if kwargs.get("use_here_match") is False or kwargs.get("use_event_annotate") is False:
        use_stamp_new_here = False
    return _make062(*args, use_stamp_new_here=use_stamp_new_here, **kwargs)


def _w_flags(w_files: list[str], w_dir: Path) -> dict[str, Any]:
    flags = _w_flags062(w_files, w_dir)
    flags["use_stamp_new_here"] = True
    return flags


def classify_common(m: dict[str, Any]) -> tuple[str, str] | None:
    saved = m.get("use_stamp_new_here")
    m["use_stamp_new_here"] = False
    early = _classify_common062(m)
    m["use_stamp_new_here"] = saved
    if early:
        return early
    if not saved:
        return "Fail", "New-here stamp was frozen off."
    return None
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if label == "Store-works":
        return (
            "Store-works",
            "New-here stamp: never-wipe English train S still PRESS from push; C life on that S: A PRESS, C TUNE. Cortex frozen.",
        )
    return label, why
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if label == "Store-works":
        return (
            "Store-works",
            "Shared return new-here stamp; dirty English S two facts; cortex frozen.",
        )
    return label, why
...
def run_tm063(seed: int = 12345, n_train: int = 500, max_steps: int = 32) -> dict[str, Any]:
    run_dir = _run_dir()
    w_a = run_dir / "W_a"
    ...
    (run_dir / "summary.md").write_text(
        f"""# TM.0.6.3 A new-here stamp vs B shared return

| Arm | Classification | Train last 50 |
|-----|----------------|---------------|
| A split new-here stamp | **{a['classification']}** | {a['train_return_last50']:.2f} |
...
def main() -> None:
    p = argparse.ArgumentParser(description="TM.0.6.3 new-here stamp on never-wipe English")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    p.add_argument("--max-steps", type=int, default=32)
    args = p.parse_args()
    m = run_tm063(seed=args.seed, n_train=args.n_train, max_steps=args.max_steps)
    print(
        json.dumps(
