"""TM.0.23.CORTEX.V7.GATE — D0–D2 with statistical paired baselines."""
...
from experiments.cortex_develop_life import LifeSeeds, bind_life_actuators, curriculum_tokens, motor_latent, pair_seeds, teach_loop
from experiments.cortex_mact_boundary import _frozen_pref_counts
from experiments.cortex_v2_gate import THRESHOLDS
from experiments.cortex_v7_scorers import score_d0_v7, score_d1_v7, score_d2_v7
...
def run_gate_life(seeds: LifeSeeds, *, device: str = "cpu") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"v7gate_{seeds.role}_") as tmp:
        ag = make_cortex(Path(tmp) / "s", genome=seeds.genome(), device=device)
        toks = curriculum_tokens(seeds)
        stages: dict[str, Any] = {}
        stages["D0"] = score_d0_v7(ag, seeds, toks)
        bind_life_actuators(ag, toks, seeds)
        birth = _frozen_pref_counts(ag, toks, 40, [toks["a"]])
        press_birth = int(birth.get(toks["press"], 0))
        teach_loop(
            ag,
            seeds,
            n=30,
        ...
        stages["D1"] = score_d1_v7(ag, seeds, toks, press_birth=press_birth)
        stages["D2"] = score_d2_v7(ag, seeds, toks)
...
def run_v7_gate_battery(*, n_pairs: int = 16, device: str = "cpu") -> dict[str, Any]:
