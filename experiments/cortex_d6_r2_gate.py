"""TM.0.23.CORTEX.D6.R2.GATE — historical score_d6 after D1–D5 prefix."""
...
from experiments.cortex_d6_r2_worlds import sealed_pair_seeds
from experiments.cortex_develop_life import bind_life_actuators, curriculum_tokens, motor_latent, teach_loop
from experiments.cortex_develop_scorers import score_d0, score_d1, score_d2, score_d3, score_d4, score_d5, score_d6
...
def run_gate_life(seeds, *, device: str = "cpu") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"d6r2_{seeds.role}_") as tmp:
        ...
        return {
            ...
            "d6_ok": bool(d6.get("ok")),
            "stages": {"D0": d0, "D1": d1, "D2": d2, "D3": d3, "D4": d4, "D5": d5, "D6": d6},
            "eligible_pair_member": bool(d0.get("ok") and d6.get("ok")),
        }
...
def run_d6_r2_gate_battery(*, seed_hex: str, n_pairs: int = 16, device: str = "cpu") -> dict[str, Any]:
    pairs = [run_gate_pair(i, seed_hex, device=device) for i in range(n_pairs)]
    n_clear = sum(1 for p in pairs if p["pair_clear"])
    stage_counts = {s: 0 for s in ("D0", "D1", "D2", "D3", "D4", "D5", "D6")}
    for p in pairs:
