"""
Stage A Reference Birth R2 search runner (lexicographic ES + separated teacher credit).
"""
...
from experiments.dev1.reference_birth_r2_preflight import run_reference_birth_r2_preflight
...
def run_stage_a_reference_birth_r2_search(
    run_id: str,
    world_seeds: list[str],
    confirmation_seeds: list[str],
    output_dir: str = "runs/exos_dev1/stage_a_reference_birth_r2",
    preflight_seed: str = "reference_birth_r2_excluded_preflight_20260820",
    ...
    _atomic_write_json(out / "run_started.json", {
        ...
        "outer_optimizer": "batched_evolution_strategies_lexicographic",
        "generations": generations,
        "population_size": population_size,
        "r1_confirmation_sealed": True,
        "r2_confirmation_preregistered_separately": True,
    })

    preflight = run_reference_birth_r2_preflight(seed=preflight_seed)
    _atomic_write_json(out / "reference_birth_r2_preflight.json", asdict(preflight))
    if not preflight.passed:
        if not preflight.checks.get("search_surface_sensitivity", True):
            outcome = "search_surface_sensitivity_fail"
        elif not preflight.checks.get("teacher_credit_separation", True):
            outcome = "teacher_credit_separation_fail"
        else:
    ...
    for arm in TREATMENT_ARMS:
        def _eval(surface, _arm=arm):
            ...
            return SurfaceIndividual(
                ...
                metrics={
                    ...
                    "selection_excludes_update_norm": True,
                },
            )

        es = run_batched_es_lexicographic(
            _eval,
            generations=generations,
            population_size=population_size,
        ...
        if not interv["treatment_outperforms_interventions"]:
            ...
            decision = "reference_birth_r2_validation_fail"
        else:
    ...
    summary = {
        "outcome": "reference_birth_r2_validation_complete",
