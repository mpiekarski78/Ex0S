"""
Developmental Birth R4-R2 life evaluation on certified Nursery Body v2.

Matched 2×2 factorial. Primary scored episode success is end_in_zone
(ever_reached and distance_reduction reported independently). Tick-fraction
comfort is retired as a primary gate.
"""
...
@dataclass
class R4R2LifeMetrics:
    development: str
    credit: str
    treatment_accuracy: float  # end_in_zone_rate (primary)
...
def evaluate_r4_r2_life(
    ...
    *,
    ...
    end_rate = float(agg["end_in_zone_rate"])
    ...
    return R4R2LifeMetrics(
        development=development,
        credit=credit,
        treatment_accuracy=end_rate,
        end_in_zone_rate=end_rate,
        ever_reached_rate=float(agg["ever_reached_rate"]),
        distance_reduction=float(agg["distance_reduction"]),
        mean_behavioral_score=score_sum / max(1, tick_total),
        mean_organism_valence=valence_sum / max(1, tick_total),
...
def evaluate_matched_factorial(
    ...
    *,
    ...
    embryonic_seed: int = 0,
    device: torch.device | None = None,
) -> dict[str, R4R2LifeMetrics]:
    out: dict[str, R4R2LifeMetrics] = {}
    for development, credit in FACTORIAL_CELLS:
        key = f"{development}__{credit}"
        out[key] = evaluate_r4_r2_life(
            development,
            credit,
            world_seed,
            n_episodes=n_episodes,
...
# Re-export ceiling helpers for preflight imports.
__all_ceiling__ = (
    "evaluate_ceiling_on_nursery_world",
    "evaluate_ceiling_gate_bundle",
)
