"""TM.0.23.CORTEX.D3.R2 — birth/candidate with 3-symbol equal-evidence HOLD."""
...
from experiments.cortex_d3_r2_gate import run_d3_r2_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.v14.lock"
STAT = DOCS / "cortex_d3_r2_stat_contract.lock"
AMEND = DOCS / "cortex_d3_r2_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d3_r2_birth.lock"
MATH = DOCS / "cortex_d3_r2_math_audit.lock"
...
PREREG = DOCS / "cortex_d3_r2.prereg.lock"
SEALED = DOCS / "cortex_d3_r2_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d3_r2_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d3_r2_gate.lock"
GATE_MD = DOCS / "tm023cortex_d3_r2_gate_results.md"
FAIL = DOCS / "cortex_d3_r2_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v15.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v15_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d3r2birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D3.R2.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_v14_sha": _sha_file(DIAG),
        ...
        "prior_v14_neural_sha": json.loads(CANDIDATE_V14.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D3.R2 three-symbol equal-evidence HOLD. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D3.R2 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("min_symbols_predicate_present", "EQUAL_EVIDENCE_MIN_SYMBOLS" in src and "len(self._symbol_obs_counts)" in src)
    ...
    add("neural_changed_from_v14", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V14.read_text(encoding="utf-8"))["neural_cortex_sha"])
    add("scorers_untouched", SCORERS_PY.exists())
    out = {
        "version": "TM.0.23.CORTEX.D3.R2.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V15.exists():
        raise RuntimeError("candidate.v15 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V15",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v14_sha": _sha_file(CANDIDATE_V14),
        "d3_r2_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v15 / D3.R2. Must re-earn C4/C5/C6 then D3. No FULLDEV reopen yet.",
    }
    CANDIDATE_V15.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v15_sha": _sha_file(CANDIDATE_V15)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V15.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V15",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v15.lock",
        "candidate_sha": _sha_file(CANDIDATE_V15),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v15 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v15)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D3.R2.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v15_sha": _sha_file(CANDIDATE_V15),
        "candidate_git_sha": git_sha,
        "mact_boundary_v15_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V15.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d3_r2_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D3.R2.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if write_lock:
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D3.R2.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
