"""TM.0.23.CORTEX.V10.GATE — D1 press/harm floors; extras scored as a population."""
...
from experiments.cortex_v10_scorers import population_d1, population_d2, score_d0_v10, score_d1_v10, score_d2_v10
from experiments.cortex_v10_worlds import sealed_pair_seeds
...
def run_gate_life(seeds: LifeSeeds, *, device: str = "cpu") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"v10gate_{seeds.role}_") as tmp:
        ag = make_cortex(Path(tmp) / "s", genome=seeds.genome(), device=device)
        toks = curriculum_tokens(seeds)
        stages: dict[str, Any] = {}
        stages["D0"] = score_d0_v10(ag, seeds, toks)
        ag.bind_actuators([toks["press"], toks["harm"]])
        birth_ckpt = ag.checkpoint()
        stages["D1"] = score_d1_v10(ag, seeds, toks, birth_ckpt=birth_ckpt)
        ag.bind_actuators([toks["press"], toks["harm"], toks["get"], toks["drop"]])
        stages["D2"] = score_d2_v10(ag, seeds, toks, birth_ckpt=birth_ckpt)
...
def run_v10_gate_battery(*, seed_hex: str, n_pairs: int = 16, device: str = "cpu") -> dict[str, Any]:
