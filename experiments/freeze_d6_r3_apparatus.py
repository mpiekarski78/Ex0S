"""Freeze isolated D6.R3 contract and commitment. No neural edits."""
...
DOCS = REPO_ROOT / "docs"
STAT_MD = DOCS / "cortex_d6_r3_stat_contract.md"
STAT_LOCK = DOCS / "cortex_d6_r3_stat_contract.lock"
GATE_MD = DOCS / "cortex_d6_r3_gate_contract.md"
AMEND_MD = DOCS / "cortex_d6_r3_architecture_amendment.md"
AMEND_LOCK = DOCS / "cortex_d6_r3_architecture_amendment.lock"
DIAG = DOCS / "cortex_diagnosis.v22.lock"
ISOL = DOCS / "cortex_d6.r3.isolation.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
CAND22 = DOCS / "cortex.candidate.v22.lock"
GATE_PY = REPO_ROOT / "experiments" / "cortex_d6_r3_gate.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_d6_r3_worlds.py"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_develop_scorers.py"
RUNNER = DOCS / "cortex_d6_r3_gate.runner.lock"
...
def freeze_d6_r3_apparatus() -> dict[str, Any]:
    if not DIAG.exists() or not ISOL.exists() or not CAND22.exists():
        raise RuntimeError("freeze v22 diagnosis + D6.R3 isolation + candidate.v22 first")
    if STAT_LOCK.exists() or PREREG.exists():
        raise RuntimeError("D6.R3 apparatus already frozen")
    cand = json.loads(CAND22.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — freeze apparatus before v23 neural edits")
    prior = []
    for name in (
    ...
    AMEND_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R3.ARCHITECTURE.AMENDMENT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "diagnosis_v22_sha": _sha(DIAG),
                ...
                "neural_sha_at_freeze": _sha(NEURAL),
                "changes_authorized": ["stronger_vocal_refractory"],
            },
            indent=2,
        )
        + "\n",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R3.GATE.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_d6_r3_gate",
    ...
    if commitment in prior:
        raise RuntimeError("D6.R3 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.D6.R3.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R3.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Narrow D6.R3. echoic emit authorized after this freeze.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_d6_r3_apparatus(), indent=2))
