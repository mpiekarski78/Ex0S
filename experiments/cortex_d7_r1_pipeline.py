"""TM.0.23.CORTEX.D7.R1 — birth/candidate with utterance persistence."""
...
from experiments.cortex_d7_r1_gate import run_d7_r1_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.fulldev_r5.lock"
STAT = DOCS / "cortex_d7_r1_stat_contract.lock"
AMEND = DOCS / "cortex_d7_r1_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d7_r1_birth.lock"
MATH = DOCS / "cortex_d7_r1_math_audit.lock"
CANDIDATE_V24 = DOCS / "cortex.candidate.v24.lock"
CANDIDATE_LIVE = DOCS / "cortex.candidate.lock"
CANDIDATE_V23 = DOCS / "cortex.candidate.v23.lock"
PREREG = DOCS / "cortex_d7_r1.prereg.lock"
SEALED = DOCS / "cortex_d7_r1_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d7_r1_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d7_r1_gate.lock"
GATE_MD = DOCS / "tm023cortex_d7_r1_gate_results.md"
FAIL = DOCS / "cortex_d7_r1_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v24.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v24_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d7r1birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D7.R1.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_fulldev_r5_sha": _sha_file(DIAG),
        ...
        "prior_v23_neural_sha": json.loads(CANDIDATE_V23.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D7.R1 utterance persistence. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D7.R1 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    from three_memory.neural_cortex import UTTERANCE_PERSIST

    add("utterance_persist", UTTERANCE_PERSIST > 0.0 and "uttering" in src)
    add("no_lifetime_max_novelty", "FAMILIARITY_RATIO * float(mx)" not in src)
    add("neural_changed_from_v23", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V23.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D7.R1.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V24.exists():
        raise RuntimeError("candidate.v24 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V24",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v23_sha": _sha_file(CANDIDATE_V23),
        "d7_r1_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v24 / D7.R1. Must re-earn C4/C5/C6 then D7. No FULLDEV reopen yet.",
    }
    CANDIDATE_V24.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v24_sha": _sha_file(CANDIDATE_V24)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V24.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V24",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v24.lock",
        "candidate_sha": _sha_file(CANDIDATE_V24),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v24 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v24)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D7.R1.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v24_sha": _sha_file(CANDIDATE_V24),
        "candidate_git_sha": git_sha,
        "mact_boundary_v24_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V24.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d7_r1_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D7.R1.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Narrow D7. Full D0–D12 stays closed unless ≥13/16.",
    }
    if write_lock:
        if GATE_LOCK.exists():
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D7.R1.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
