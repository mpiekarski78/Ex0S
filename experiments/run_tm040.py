"""TM.0.4.0: leave the door world. Channel dial + prose free life.

A split credit. B shared return.
Not the key/door room. Held-out channel C: place code equals PRESS; must copy TUNE.
"""
...
from three_memory.dial_env import (
    CH_A,
    CH_B,
    CH_C,
    CORRECT,
...
from three_memory.tag_store import ProseLibrary, TagStore, prose_to_record, write_prose_notes

WIKI_A = (
    "p99.md",
    f"Staff scrap from channel A. At place {CH_A} the working motor was {int(DialAction.PRESS)}.\n",
)
WIKI_C = (
    "p98.md",
    f"Staff scrap from channel C. At place {CH_C} the working motor was {int(DialAction.TUNE)}.\n",
)
SWAP_A = (
    "p99.md",
    f"Staff scrap from channel A. At place {CH_A} the working motor was {int(DialAction.IDLE)}.\n",
)


def _run_dir() -> Path:
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H%M%S")
    d = REPO_ROOT / "runs" / f"{stamp}_tm040"
    d.mkdir(parents=True, exist_ok=True)
    return d


def clutter_prose() -> list[tuple[str, str]]:
    """Clutter like TM.0.3.2: some pages carry place codes, never the A/C answer motors as motors.

    Digits may include PRESS=1 as a *place* (channel C code) so untrained/vname noise can
    accidentally PRESS — same structural accident as door clutter planting USE_KEY as place=2.
    Never plant TUNE=3. Per-file found_a_pair still requires {0,1} together (only p99).
    """
    ...
    for i in range(11):
        if i % 3 == 0:
            ...
            place = CH_C  # 1 == PRESS; accidental motor if vname copies place
        else:
            place = 8 + (i % 2)
        motor = int(DialAction.HOLD) if i % 2 == 0 else int(DialAction.FLIP)
        assert motor not in (int(DialAction.PRESS), int(DialAction.TUNE))
        notes.append(
            (
                f"c{i:02d}.md",
                f"Clutter bench notes. Place {place} is noisy; idle motor {motor}.\n",
            )
        )
    return notes
...
def _s_has_pair(tag: str, place: int, motor: int) -> bool:
    """True if one committed file carries both place and motor ints."""
    blocks: list[set[int]] = []
    cur: set[int] = set()
    ...
    if cur:
        blocks.append(cur)
    return any(place in b and motor in b for b in blocks)


def _s_has_a_pair(tag: str) -> bool:
    return _s_has_pair(tag, CH_A, int(DialAction.PRESS))


def _s_has_c_pair(tag: str) -> bool:
    return _s_has_pair(tag, CH_C, int(DialAction.TUNE))
...
def probe(
    ...
    correct = action == int(need) and bool(result.info.get("opened"))
    return {
        "scenario": scenario,
        "action": int(action),
...
def live_free(
    ...
    *,
    ...
    return {
        ...
        "found_a_pair": _s_has_a_pair(tag),
        "found_c_pair": _s_has_c_pair(tag),
        "steps": log,
    }
...
def classify_common(m: dict[str, Any]) -> tuple[str, str] | None:
    ...
    if CH_A not in m.get("w_a_ints", []) or int(DialAction.PRESS) not in m.get("w_a_ints", []):
        return "Confound", "Useful prose must contain channel A code and PRESS as digits."
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if m["untrained_live"].get("found_a_pair"):
        return "Fail", "Untrained already found the useful A page."
    ...
    if not m["c_after_reset"]["correct"]:
        return "Fail", "Held-out channel C failed; place-copy or no transfer."
    if m["c_after_reset"]["action_name"] != "tune":
        return "Fail", "Held-out C did not TUNE (place-copy trap)."
    if m["empty_S"]["correct"]:
        return "Fail", "Empty S still solved A."
    if m["empty_S_c"]["correct"] or m["empty_S_c"]["action_name"] == "tune":
        return "Fail", "Empty S already TUNE on held-out C."
    ...
    if not m["a_live"]["found_a_pair"]:
        return "Fail", "Free A life never committed place+PRESS ints."
    if not m["c_live"]["found_c_pair"]:
        return "Fail", "Held-out C life never committed place+TUNE ints."
    tag = m.get("a_tag", "")
    if _has_field(tag, "action") or _has_field(tag, "door") or _has_field(tag, "where"):
        return "Fail", "S restored filed tag names."
    return (
        "Store-works",
        "Channel dial free life; after ρ reset W gone, A PRESS from S; held-out C TUNE not place-copy; cortex frozen.",
    )
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    return (
        "Store-works",
        "Shared return on channel dial free life; cortex frozen; held-out C TUNE.",
    )
...
def _train(
    ...
    *,
    ...
    for ep in range(n):
        ...
        r_found = 1.0 if live["found_a_pair"] else 0.0
        ag.world = None
        ag.reset_rho()
...
def run_tm040(seed: int = 12345, n_train: int = 500, max_steps: int = 32) -> dict[str, Any]:
    run_dir = _run_dir()
    w_a = run_dir / "W_a"
    w_c = run_dir / "W_c"
    ...
    (run_dir / "summary.md").write_text(
        f"""# TM.0.4.0 A channel dial vs B shared return

| Arm | Classification | Train last 50 |
|-----|----------------|---------------|
| A split dial | **{a['classification']}** | {a['train_return_last50']:.2f} |
...
def main() -> None:
    p = argparse.ArgumentParser(description="TM.0.4.0 channel dial; leave the door world")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    p.add_argument("--max-steps", type=int, default=32)
    args = p.parse_args()
    m = run_tm040(seed=args.seed, n_train=args.n_train, max_steps=args.max_steps)
    print(
        json.dumps(
