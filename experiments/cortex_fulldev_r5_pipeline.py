"""TM.0.23.CORTEX.FULLDEV.R5 — reveal and score D0–D12."""
...
from experiments.cortex_fulldev_r5 import run_fulldev_r5_battery
...
REVEAL = DOCS / "cortex_fulldev_r5_eval_reveal.lock"
RUNNER = DOCS / "cortex_fulldev_r5.runner.lock"
RESULT = DOCS / "cortex_fulldev_r5.lock"
RESULT_MD = DOCS / "tm023cortex_fulldev_r5_results.md"
FAIL = DOCS / "cortex_fulldev_r5.failure.lock"
DEV_V13 = DOCS / "cortex_development.v13.lock"
CANDIDATE_GIT = "09ca83436dca2d9d4b54d0a67b1210f0e52877da"
...
def reveal_fulldev_r5() -> dict[str, Any]:
    if not PREREG.exists() or not RUNNER.exists() or not CANDIDATE_V23.exists():
        raise RuntimeError("FULLDEV.R5 apparatus + candidate v23 required")
    if DEV_V13.exists():
        raise RuntimeError("DEVELOP.v13 exists — refuse")
    if not _git_clean():
        raise RuntimeError("working tree dirty — refuse reveal")
    if REVEAL.exists():
        raise RuntimeError("FULLDEV.R5 reveal exists")
    git_sha = _git_head()
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.FULLDEV.R5.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def run_fulldev_r5(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    ...
    battery = run_fulldev_r5_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.FULLDEV.R5.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Full D0–D12 on TM023.FULL.R5. worlds. Does not stamp 0.0.005.",
    }
    if write_lock:
        if RESULT.exists():
            raise RuntimeError("FULLDEV.R5 result exists")
        RESULT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        RESULT_MD.write_text(
            "# TM.0.23.CORTEX FULLDEV.R5\n\n"
        ...
        if not battery["development_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.FULLDEV.R5.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
                        ...
                        "refuse": [
                            "edit-and-rescore on revealed FULLDEV.R5 worlds",
                            "stamp earned_next",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    if args.reveal:
        print(json.dumps(reveal_fulldev_r5(), indent=2))
    elif args.score:
        print(json.dumps(run_fulldev_r5(device=args.device, write_lock=args.write_lock), indent=2, default=str))
    else:
        ap.print_help()
