"""Freeze isolated D6.R1 contract and commitment. No neural edits."""
...
DOCS = REPO_ROOT / "docs"
STAT_MD = DOCS / "cortex_d6_r1_stat_contract.md"
STAT_LOCK = DOCS / "cortex_d6_r1_stat_contract.lock"
GATE_MD = DOCS / "cortex_d6_r1_gate_contract.md"
AMEND_MD = DOCS / "cortex_d6_r1_architecture_amendment.md"
AMEND_LOCK = DOCS / "cortex_d6_r1_architecture_amendment.lock"
DIAG = DOCS / "cortex_diagnosis.fulldev_r4.lock"
ISOL = DOCS / "cortex_d6.isolation.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
CAND20 = DOCS / "cortex.candidate.v20.lock"
GATE_PY = REPO_ROOT / "experiments" / "cortex_d6_r1_gate.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_d6_r1_worlds.py"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_develop_scorers.py"
RUNNER = DOCS / "cortex_d6_r1_gate.runner.lock"
...
def freeze_d6_r1_apparatus() -> dict[str, Any]:
    if not DIAG.exists() or not ISOL.exists() or not CAND20.exists():
        raise RuntimeError("freeze v20 diagnosis + D6.R1 isolation + candidate.v20 first")
    if STAT_LOCK.exists() or PREREG.exists():
        raise RuntimeError("D6.R1 apparatus already frozen")
    cand = json.loads(CAND20.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — freeze apparatus before v21 neural edits")
    prior = []
    for name in (
    ...
    AMEND_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R1.ARCHITECTURE.AMENDMENT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "diagnosis_fulldev_r4_sha": _sha(DIAG),
                ...
                "neural_sha_at_freeze": _sha(NEURAL),
                "changes_authorized": ["echoic_emit"],
            },
            indent=2,
        )
        + "\n",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R1.GATE.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_d6_r1_gate",
    ...
    if commitment in prior:
        raise RuntimeError("D6.R1 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.D6.R1.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R1.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Narrow D6.R1. echoic emit authorized after this freeze.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_d6_r1_apparatus(), indent=2))
