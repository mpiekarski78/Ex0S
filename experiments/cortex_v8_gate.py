"""TM.0.23.CORTEX.V8.GATE — D0–D2 with seed-derived lives and birth-weight extras."""
...
from experiments.cortex_v8_scorers import score_d0_v8, score_d1_v8, score_d2_v8
from experiments.cortex_v8_worlds import sealed_pair_seeds
...
def run_gate_life(seeds: LifeSeeds, *, device: str = "cpu") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"v8gate_{seeds.role}_") as tmp:
        ag = make_cortex(Path(tmp) / "s", genome=seeds.genome(), device=device)
        toks = curriculum_tokens(seeds)
        stages: dict[str, Any] = {}
        stages["D0"] = score_d0_v8(ag, seeds, toks)
        bind_life_actuators(ag, toks, seeds)
        birth_ckpt = ag.checkpoint()
        stages["D1"] = score_d1_v8(ag, seeds, toks, birth_ckpt=birth_ckpt)
        stages["D2"] = score_d2_v8(ag, seeds, toks, birth_ckpt=birth_ckpt)
...
def run_v8_gate_battery(*, seed_hex: str, n_pairs: int = 16, device: str = "cpu") -> dict[str, Any]:
