...
Same corrected concurrent-bind English store as TM.0.6.6. A new station
commits the attended rare page, or stamps nothing. Walking unread W in file
order is not a CS. Not a ranker, not unique-rare restored, not math, not
dropping has_code or domain="dial", not solving B.
"""
...
from experiments.run_tm066 import (
    MAX_TRAIN_S_FILES,
    classify_a as _classify_a066,
    classify_b as _classify_b066,
    classify_common as _classify_common066,
    make as _make066,
    run_arm as _run_arm066,
    _w_flags as _w_flags066,
)
...
def _run_dir() -> Path:
    ...
    d = REPO_ROOT / "runs" / f"{stamp}_tm067"
    d.mkdir(parents=True, exist_ok=True)
    return d


def make(*args, use_in_hand_new_here: bool = True, **kwargs):
    if kwargs.get("use_here_match") is False or kwargs.get("use_event_annotate") is False:
        use_in_hand_new_here = False
    return _make066(*args, use_in_hand_new_here=use_in_hand_new_here, **kwargs)


def _w_flags(w_files: list[str], w_dir: Path) -> dict[str, Any]:
    flags = _w_flags066(w_files, w_dir)
...
def _hide_in_hand(m: dict[str, Any]) -> Any:
    saved = m.get("use_in_hand_new_here")
    m["use_in_hand_new_here"] = False
    return saved


def classify_common(m: dict[str, Any]) -> tuple[str, str] | None:
    saved = _hide_in_hand(m)
    early = _classify_common066(m)
    m["use_in_hand_new_here"] = saved
    if early:
        return early
    if not saved:
        return "Fail", "In-hand new-here was frozen off."
    if not m.get("in_hand_new_here"):
        return "Fail", "In-hand new-here was frozen off."
    return None
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if label == "Store-works":
        return (
            "Store-works",
            "In-hand new-here: corrected English S stamps the attended rare page at a new station, not the first leftover hapax; train S PRESS from push; C life A PRESS, C TUNE. Cortex frozen.",
        )
    return label, why
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if label == "Store-works":
        return (
            "Store-works",
            "Shared return in-hand new-here; small store two facts; cortex frozen.",
        )
    return label, why


def run_arm(**kwargs: Any) -> dict[str, Any]:
    import experiments.run_tm066 as tm066

    saved = (tm066.make, tm066.classify_a, tm066.classify_b, tm066._w_flags)
    tm066.make = make
    tm066.classify_a = classify_a
    tm066.classify_b = classify_b
    tm066._w_flags = _w_flags
    try:
        m = _run_arm066(**kwargs)
    finally:
        tm066.make, tm066.classify_a, tm066.classify_b, tm066._w_flags = saved
    m["use_in_hand_new_here"] = True
    m["in_hand_new_here"] = True
...
def run_tm067(seed: int = 12345, n_train: int = 500, max_steps: int = 32) -> dict[str, Any]:
    run_dir = _run_dir()
    w_a = run_dir / "W_a"
    ...
    (run_dir / "summary.md").write_text(
        f"""# TM.0.6.7 A in-hand new-here vs B shared return

| Arm | Classification | Train last 50 |
|-----|----------------|---------------|
| A split in-hand new-here | **{a['classification']}** | {a['train_return_last50']:.2f} |
...
def main() -> None:
    p = argparse.ArgumentParser(description="TM.0.6.7 in-hand new-here, not leftover rare")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    p.add_argument("--max-steps", type=int, default=32)
    args = p.parse_args()
    m = run_tm067(seed=args.seed, n_train=args.n_train, max_steps=args.max_steps)
    print(
        json.dumps(
