"""Freeze FULLDEV.R4 commitment. No neural edits. No score."""
...
DOCS = REPO_ROOT / "docs"
CONTRACT_MD = DOCS / "cortex_fulldev_r4_contract.md"
CONTRACT_LOCK = DOCS / "cortex_fulldev_r4_contract.lock"
CLEAR = DOCS / "cortex_d5_r2_gate.clear.note.lock"
CAND20 = DOCS / "cortex.candidate.v20.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
GATE_PY = REPO_ROOT / "experiments" / "cortex_fulldev_r4.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_fulldev_r4_worlds.py"
...
def freeze_fulldev_r4_apparatus() -> dict[str, Any]:
    if not CLEAR.exists() or not CAND20.exists():
        raise RuntimeError("D5.R2 clear + candidate v20 required")
    if PREREG.exists() or RUNNER.exists():
        raise RuntimeError("FULLDEV.R4 apparatus already frozen")
    cand = json.loads(CAND20.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — freeze apparatus on the v20 neural")
    prior = []
    for name in (
    ...
    CONTRACT_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.FULLDEV.R4.CONTRACT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "contract_md_sha": _sha(CONTRACT_MD),
                "development_contract_sha": _sha(DEV_CONTRACT),
                "d5_r2_clear_note_sha": _sha(CLEAR),
                "candidate_v20_sha": _sha(CAND20),
                ...
                "refuse": [
                    "pair_seeds table",
                    "TM023.FULL.R1. / FULL.R2. / FULL.R3. worlds",
                    "D3.R* / D4.R* / D5.R* worlds",
                    "TM023.V8–V20 worlds",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.FULLDEV.R4.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_fulldev_r4",
    ...
    if commitment in prior:
        raise RuntimeError("FULLDEV.R4 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.FULLDEV.R4.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.FULLDEV.R4.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Full D0–D12 on unused sealed worlds after isolated D3.R3 and D5.R2 clears. Not a product stamp.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_fulldev_r4_apparatus(), indent=2))
