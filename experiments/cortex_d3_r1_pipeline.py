"""TM.0.23.CORTEX.D3.R1 — birth/candidate with equal-evidence / low-familiarity HOLD."""
...
from experiments.cortex_d3_r1_gate import run_d3_r1_gate_battery
...
from three_memory.neural_cortex import CONFLICT_HOLD_BIAS, FAMILIARITY_RATIO, MOTOR_ACT_TOKENS, OP_COST, OPS, GenomeConfig
...
DIAG = DOCS / "cortex_diagnosis.fulldev_r1.lock"
STAT = DOCS / "cortex_d3_r1_stat_contract.lock"
AMEND = DOCS / "cortex_d3_r1_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d3_r1_birth.lock"
MATH = DOCS / "cortex_d3_r1_math_audit.lock"
CANDIDATE_V14 = DOCS / "cortex.candidate.v14.lock"
CANDIDATE_LIVE = DOCS / "cortex.candidate.lock"
CANDIDATE_V13 = DOCS / "cortex.candidate.v13.lock"
PREREG = DOCS / "cortex_d3_r1.prereg.lock"
SEALED = DOCS / "cortex_d3_r1_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d3_r1_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d3_r1_gate.lock"
GATE_MD = DOCS / "tm023cortex_d3_r1_gate_results.md"
FAIL = DOCS / "cortex_d3_r1_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v14.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v14_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d3birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D3.R1.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_fulldev_r1_sha": _sha_file(DIAG),
        ...
        "prior_v13_neural_sha": json.loads(CANDIDATE_V13.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D3.R1 equal-evidence / low-familiarity HOLD. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D3.R1 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("cue_hold_present", "_symbol_obs_counts" in src and "FAMILIARITY_RATIO" in src)
    add("neural_changed_from_v13", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V13.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {"version": "TM.0.23.CORTEX.D3.R1.MATH.AUDIT", "product": "0.0.004", "earned_next": False, "ex0s": None, "ok": all(c["ok"] for c in checks), "checks": checks}
    if MATH.exists():
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V14.exists():
        raise RuntimeError("candidate.v14 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V14",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v13_sha": _sha_file(CANDIDATE_V13),
        "d3_r1_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v14 / D3.R1. Must re-earn D3. No FULLDEV reopen yet.",
    }
    CANDIDATE_V14.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v14_sha": _sha_file(CANDIDATE_V14)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V14.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V14",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v14.lock",
        "candidate_sha": _sha_file(CANDIDATE_V14),
        "required_ids": ["C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"],
        "all_required_green": all(next(x for x in results if x["id"] == i).get("ok") for i in ("C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population")),
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D3.R1.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v14_sha": _sha_file(CANDIDATE_V14),
        "candidate_git_sha": git_sha,
        "mact_boundary_v14_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V14.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d3_r1_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D3.R1.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Narrow D3. Full D0–D12 stays closed unless ≥13/16.",
    }
    if write_lock:
        if GATE_LOCK.exists():
            raise RuntimeError("gate lock exists")
        GATE_LOCK.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        GATE_MD.write_text(f"# TM.0.23.CORTEX D3.R1 gate\n\n**relation_gate_clear:** `{battery['relation_gate_clear']}`\n**n_pair_clear:** `{battery['n_pair_clear']}/16`\n\n", encoding="utf-8")
        if not battery["relation_gate_clear"]:
            FAIL.write_text(json.dumps({"version": "TM.0.23.CORTEX.D3.R1.GATE.FAILURE", "product": "0.0.004", "earned_next": False, "ex0s": None, "gate_sha": _sha_file(GATE_LOCK), "n_pair_clear": battery["n_pair_clear"], "refuse": ["reopen FULLDEV", "edit-and-rescore", "nursery"]}, indent=2) + "\n", encoding="utf-8")
        summary["locks_written"] = True
    return summary
