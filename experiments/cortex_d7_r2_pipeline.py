"""TM.0.23.CORTEX.D7.R2 — birth/candidate with phrase-final STOP."""
...
from experiments.cortex_d7_r2_gate import run_d7_r2_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.v24.lock"
STAT = DOCS / "cortex_d7_r2_stat_contract.lock"
AMEND = DOCS / "cortex_d7_r2_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d7_r2_birth.lock"
MATH = DOCS / "cortex_d7_r2_math_audit.lock"
...
PREREG = DOCS / "cortex_d7_r2.prereg.lock"
SEALED = DOCS / "cortex_d7_r2_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d7_r2_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d7_r2_gate.lock"
GATE_MD = DOCS / "tm023cortex_d7_r2_gate_results.md"
FAIL = DOCS / "cortex_d7_r2_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v25.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v25_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d7r2birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D7.R2.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_v24_sha": _sha_file(DIAG),
        ...
        "prior_v24_neural_sha": json.loads(CANDIDATE_V24.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D7.R2 phrase-final STOP. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D7.R2 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("phrase_final_stop", "_phrase" in src and "phrase_target" in src and "op == \"STOP\"" in src)
    add("no_softmax_utterance_persist", "if uttering:" not in src or "UTTERANCE_PERSIST" not in src.split("if uttering:")[1][:200])
    add("no_lifetime_max_novelty", "FAMILIARITY_RATIO * float(mx)" not in src)
    add("neural_changed_from_v24", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V24.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D7.R2.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V25.exists():
        raise RuntimeError("candidate.v25 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V25",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v24_sha": _sha_file(CANDIDATE_V24),
        "d7_r2_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v25 / D7.R2. Must re-earn C4/C5/C6 then D7. No FULLDEV reopen yet.",
    }
    CANDIDATE_V25.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v25_sha": _sha_file(CANDIDATE_V25)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V25.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V25",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v25.lock",
        "candidate_sha": _sha_file(CANDIDATE_V25),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v25 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v25)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D7.R2.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "mact_boundary_v25_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    ...
    battery = run_d7_r2_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D7.R2.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if write_lock:
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D7.R2.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
