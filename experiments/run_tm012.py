"""TM.0.1.2: messy retrieve. Rank unread files; no exact loc=/door= query.
...
def _run_dir() -> Path:
    ...
    d = REPO_ROOT / "runs" / f"{stamp}_tm012"
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if not m["red_unmount"]["correct"]:
        return "Fail", "Search did not find the messy unread page after unmount W."
    ...
    return (
        "Store-works",
        "Messy retrieve, no exact loc=/door=; cortex frozen; unmount W; held-out green wait; controls fail.",
    )
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    return (
        "Store-works",
        "Shared return solved messy retrieve; cortex frozen; unmount W; held-out green wait.",
    )
...
def run_tm012(seed: int = 12345, n_train: int = 500) -> dict[str, Any]:
    run_dir = _run_dir()
    w_red = run_dir / "W_red"
    ...
    (run_dir / "summary.md").write_text(
        f"""# TM.0.1.2 A messy retrieve vs B shared return

| Arm | Classification | Train last 50 |
|-----|----------------|---------------|
| A split messy retrieve | **{a['classification']}** | {a['train_return_last50']:.2f} |
...
def main() -> None:
    p = argparse.ArgumentParser(description="TM.0.1.2 messy retrieve; split vs shared return")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    args = p.parse_args()
    m = run_tm012(seed=args.seed, n_train=args.n_train)
