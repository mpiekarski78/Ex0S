"""Freeze isolated D6.R2 contract and commitment. No neural edits."""
...
DOCS = REPO_ROOT / "docs"
STAT_MD = DOCS / "cortex_d6_r2_stat_contract.md"
STAT_LOCK = DOCS / "cortex_d6_r2_stat_contract.lock"
GATE_MD = DOCS / "cortex_d6_r2_gate_contract.md"
AMEND_MD = DOCS / "cortex_d6_r2_architecture_amendment.md"
AMEND_LOCK = DOCS / "cortex_d6_r2_architecture_amendment.lock"
DIAG = DOCS / "cortex_diagnosis.v21.lock"
ISOL = DOCS / "cortex_d6.r2.isolation.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
CAND21 = DOCS / "cortex.candidate.v21.lock"
GATE_PY = REPO_ROOT / "experiments" / "cortex_d6_r2_gate.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_d6_r2_worlds.py"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_develop_scorers.py"
RUNNER = DOCS / "cortex_d6_r2_gate.runner.lock"
...
def freeze_d6_r2_apparatus() -> dict[str, Any]:
    if not DIAG.exists() or not ISOL.exists() or not CAND21.exists():
        raise RuntimeError("freeze v21 diagnosis + D6.R2 isolation + candidate.v21 first")
    if STAT_LOCK.exists() or PREREG.exists():
        raise RuntimeError("D6.R2 apparatus already frozen")
    cand = json.loads(CAND21.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — freeze apparatus before v22 neural edits")
    prior = []
    for name in (
    ...
    AMEND_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R2.ARCHITECTURE.AMENDMENT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "diagnosis_v21_sha": _sha(DIAG),
                ...
                "neural_sha_at_freeze": _sha(NEURAL),
                "changes_authorized": ["vocal_refractory"],
            },
            indent=2,
        )
        + "\n",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R2.GATE.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_d6_r2_gate",
    ...
    if commitment in prior:
        raise RuntimeError("D6.R2 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.D6.R2.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.D6.R2.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Narrow D6.R2. echoic emit authorized after this freeze.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_d6_r2_apparatus(), indent=2))
