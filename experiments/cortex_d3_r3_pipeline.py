"""TM.0.23.CORTEX.D3.R3 — birth/candidate with current-observe body_adv equal-evidence HOLD."""
...
from experiments.cortex_d3_r3_gate import run_d3_r3_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.v15.lock"
STAT = DOCS / "cortex_d3_r3_stat_contract.lock"
AMEND = DOCS / "cortex_d3_r3_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d3_r3_birth.lock"
MATH = DOCS / "cortex_d3_r3_math_audit.lock"
CANDIDATE_V16 = DOCS / "cortex.candidate.v16.lock"
CANDIDATE_LIVE = DOCS / "cortex.candidate.lock"
CANDIDATE_V15 = DOCS / "cortex.candidate.v15.lock"
PREREG = DOCS / "cortex_d3_r3.prereg.lock"
SEALED = DOCS / "cortex_d3_r3_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d3_r3_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d3_r3_gate.lock"
GATE_MD = DOCS / "tm023cortex_d3_r3_gate_results.md"
FAIL = DOCS / "cortex_d3_r3_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v16.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v16_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d3r3birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D3.R3.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_v15_sha": _sha_file(DIAG),
        ...
        "prior_v15_neural_sha": json.loads(CANDIDATE_V15.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D3.R3 current-observe body_adv equal-evidence HOLD. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D3.R3 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("current_body_adv_predicate", "cur_body_adv" in src and "body_prev - BODY_SETPOINT" in src)
    add("not_keyed_on_last_act", "abs(cur_body_adv) <= CONFLICT_ADV_EPS" in src)
    add("c6_safe_predicate_present", "len(self._symbol_obs_counts) >= EQUAL_EVIDENCE_MIN_SYMBOLS" in src)
    add("cue_hold_present", "_symbol_obs_counts" in src and "FAMILIARITY_RATIO" in src)
    add("neural_changed_from_v15", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V15.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D3.R3.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V16.exists():
        raise RuntimeError("candidate.v16 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V16",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v15_sha": _sha_file(CANDIDATE_V15),
        "d3_r3_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v16 / D3.R3. Must re-earn C4/C5/C6 then D3. No FULLDEV reopen yet.",
    }
    CANDIDATE_V16.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v16_sha": _sha_file(CANDIDATE_V16)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V16.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V16",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v16.lock",
        "candidate_sha": _sha_file(CANDIDATE_V16),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v16 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v16)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D3.R3.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v16_sha": _sha_file(CANDIDATE_V16),
        "candidate_git_sha": git_sha,
        "mact_boundary_v16_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V16.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d3_r3_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D3.R3.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if write_lock:
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D3.R3.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
