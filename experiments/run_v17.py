"""v17: A read do= vs action=. B match here= vs door=."""
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if any(p.get("explored") for p in (m["untrained_do"], m["red_probe"], m["green_probe"])):
        return "Confound", "Probe used exploration."
    if m["trained_force_use"]:
        return "Fail", "Use-gate clamped to rescue the plot."
    if not m["key_changed"]:
        return "Fail", "Read-key head did not change."
    if m["untrained_do"]["correct"] or m["untrained_do"]["action_name"] == "use_key":
        return "Fail", "Untrained already read do= (or table still wired)."
    if not m["red_probe"]["correct"]:
        return "Fail", "Trained key-head did not copy red do=2."
    if not m["green_probe"]["correct"]:
        return "Fail", "Held-out green failed; head learned that door, not read do=."
    if m["empty_S"]["correct"]:
        return "Fail", "Empty S still use_key."
    if m["action_control"]["correct"]:
        return "Fail", "Reading action= still solved red; do= was not required."
    if (m["red_probe"].get("policy") or {}).get("key_alt") is False:
        return "Fail", "Probe did not gate key_alt (do=)."
    ...
    return (
        "Store-works",
        "Read-key learned do=; cortex frozen; held-out green wait; action= control fails.",
    )
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if m["disable_S_red"]["correct"]:
        return "Confound", "disable-S still used the key; fact leaked."
    if any(p.get("explored") for p in (m["untrained_here"], m["red_probe"], m["green_probe"])):
        return "Confound", "Probe used exploration."
    if m["trained_force_use"]:
        return "Fail", "Use-gate clamped to rescue the plot."
    if not m["match_changed"]:
        return "Fail", "Match-key head did not change."
    if m["untrained_here"]["correct"] or m["untrained_here"]["action_name"] == "use_key":
        return "Fail", "Untrained already matched here= (or table still wired)."
    if not m["red_probe"]["correct"]:
        return "Fail", "Trained match-head did not use red here=."
    if not m["green_probe"]["correct"]:
        return "Fail", "Held-out green failed; head learned that door, not match here=."
    if m["empty_S"]["correct"]:
        return "Fail", "Empty S still use_key."
    if m["door_control"]["correct"]:
        return "Fail", "Matching door= still solved red; here= was not required."
    if (m["red_probe"].get("policy") or {}).get("match_alt") is False:
        return "Fail", "Probe did not gate match_alt (here=)."
    ...
    return (
        "Store-works",
        "Match-key learned here=; cortex frozen; held-out green wait; door= control fails.",
    )
...
def run_arm_a(run_dir: Path, w_clutter: Path, w_files: list[str], seed: int, n_train: int, max_steps: int) -> dict[str, Any]:
    ...
    write_tag_notes(s_un, [("d0.tag", {"door": 0, "do": 2})])
    un_a = make_a(s_un, None, policy, force_use=True)
    un_a.reset_rho()
    untrained_do = probe(un_a, "probe_red_with_key", seed + 10)

    rewards = _train(make_a, "key", policy, w_clutter, work, n_train, seed, max_steps)
...
def run_arm_b(run_dir: Path, w_clutter: Path, w_files: list[str], seed: int, n_train: int, max_steps: int) -> dict[str, Any]:
    ...
    write_tag_notes(s_un, [("d0.tag", {"here": 0, "action": 2})])
    un_a = make_b(s_un, None, policy, force_use=True)
    un_a.reset_rho()
    untrained_here = probe(un_a, "probe_red_with_key", seed + 10)

    rewards = _train(make_b, "match", policy, w_clutter, work, n_train, seed + 5, max_steps)
    ...
    ctrl.w_use = policy.w_use.copy()
    ctrl.b_use = np.array(float(policy.b_use))
    ctrl_a = make_b(s_ctrl, None, ctrl)
    ctrl_a.reset_rho()
...
def run_v17(seed: int = 12345, n_train: int = 500, max_steps: int = 32) -> dict[str, Any]:
    run_dir = _run_dir()
    w_clutter = run_dir / "W"
...
|-----|----------------|---------------|
| A read do= | **{a['classification']}** | {a['train_return_last50']:.2f} |
| B match here= | **{b['classification']}** | {b['train_return_last50']:.2f} |
...
| Check | A | B |
|-------|---|---|
| Untrained | {a['untrained_do']['action_name']} ({a['untrained_do']['correct']}) | {b['untrained_here']['action_name']} ({b['untrained_here']['correct']}) |
...
def main() -> None:
    p = argparse.ArgumentParser(description="v17 read do= vs match here=")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    p.add_argument("--max-steps", type=int, default=32)
    args = p.parse_args()
    m = run_v17(seed=args.seed, n_train=args.n_train, max_steps=args.max_steps)
    print(json.dumps({"A": m["A"]["classification"], "B": m["B"]["classification"], "run_dir": m["run_dir"]}, indent=2))
    print("A", m["A"]["untrained_do"]["action_name"], m["A"]["red_probe"]["action_name"], m["A"]["green_probe"]["action_name"], m["A"]["red_tag"].strip())
    print("B", m["B"]["untrained_here"]["action_name"], m["B"]["red_probe"]["action_name"], m["B"]["green_probe"]["action_name"], m["B"]["red_tag"].strip())
