"""TM.0.23.CORTEX.D7.R1.GATE — historical score_d7 after D1–D6 prefix."""
...
from experiments.cortex_d7_r1_worlds import sealed_pair_seeds
...
def run_gate_life(seeds, *, device: str = "cpu") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"d7r1_{seeds.role}_") as tmp:
        ...
        return {
            ...
            "d7_ok": bool(d7.get("ok")),
            "stages": {"D0": d0, "D1": d1, "D2": d2, "D3": d3, "D4": d4, "D5": d5, "D6": d6, "D7": d7},
            "eligible_pair_member": bool(d0.get("ok") and d7.get("ok")),
        }
...
def run_d7_r1_gate_battery(*, seed_hex: str, n_pairs: int = 16, device: str = "cpu") -> dict[str, Any]:
    pairs = [run_gate_pair(i, seed_hex, device=device) for i in range(n_pairs)]
    n_clear = sum(1 for p in pairs if p["pair_clear"])
    stage_counts = {s: 0 for s in ("D0", "D1", "D2", "D3", "D4", "D5", "D6", "D7")}
    for p in pairs:
