"""Freeze v11 scorer contract and commitment. No neural edits. No candidate SHA."""
...
STAT_MD = DOCS / "cortex_v11_stat_contract.md"
STAT_LOCK = DOCS / "cortex_v11_stat_contract.lock"
GATE_MD = DOCS / "cortex_v11_gate_contract.md"
AMEND_MD = DOCS / "cortex_v11_architecture_amendment.md"
AMEND_LOCK = DOCS / "cortex_v11_architecture_amendment.lock"
DIAG = DOCS / "cortex_diagnosis.v10.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
CAND10 = DOCS / "cortex.candidate.v10.lock"
GATE_PY = REPO_ROOT / "experiments" / "cortex_v11_gate.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_v11_worlds.py"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_v11_scorers.py"
RUNNER = DOCS / "cortex_v11_gate.runner.lock"
PREREG = DOCS / "cortex_v11.prereg.lock"
SEALED = DOCS / "cortex_v11_eval_secrets.sealed.json"
...
def freeze_v11_apparatus() -> dict[str, Any]:
    if not DIAG.exists():
        raise RuntimeError("freeze v10 diagnosis first")
    if STAT_LOCK.exists() or PREREG.exists():
        raise RuntimeError("v11 apparatus already frozen")
    cand = json.loads(CAND10.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — v11 is scorer-only")
    prior = []
    for name in (
    ...
    stat = {
        "version": "TM.0.23.CORTEX.V11.STAT.CONTRACT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "refuse": ["drop holds>=5", "edit historical score_d2", "edit-and-rescore v10", "new credit law"],
    }
    STAT_LOCK.write_text(json.dumps(stat, indent=2) + "\n", encoding="utf-8")
    AMEND_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V11.ARCHITECTURE.AMENDMENT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "diagnosis_v10_sha": _sha(DIAG),
                ...
                "neural_sha_at_freeze": _sha(NEURAL),
                "changes_authorized": ["retain_v10_neural", "d2_hold_on_swapped_physics"],
            },
            indent=2,
        )
        + "\n",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V11.GATE.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_v11_gate",
    ...
    if commitment in prior:
        raise RuntimeError("v11 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.V11.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V11.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Narrow v11 D1–D2. Scorer-only contradictory HOLD window. Not DEVELOP.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_v11_apparatus(), indent=2))
