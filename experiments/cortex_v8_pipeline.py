"""TM.0.23.CORTEX.V8 — birth/candidate (retain v7 neural), population boundary, seed-derived D1–D2."""
...
from experiments.cortex_v8_gate import run_v8_gate_battery
...
STAT = DOCS / "cortex_v8_stat_contract.lock"
AMEND = DOCS / "cortex_v8_architecture_amendment.lock"
V8_BIRTH = DOCS / "cortex_v8_birth.lock"
V8_MATH = DOCS / "cortex_v8_math_audit.lock"
...
V8_REVEAL = DOCS / "cortex_v8_eval_reveal.lock"
V8_GATE_PREREG = DOCS / "cortex_v8_gate.prereg.lock"
V8_GATE_RUNNER = DOCS / "cortex_v8_gate.runner.lock"
V8_GATE_LOCK = DOCS / "cortex_v8_gate.lock"
V8_GATE_MD = DOCS / "tm023cortex_v8_gate_results.md"
V8_FAIL = DOCS / "cortex_v8_gate.failure.lock"
MACT_V8 = DOCS / "cortex_mact_boundary.v8.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v8_results.md"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_v8_scorers.py"
DEV_V8 = DOCS / "cortex_development.v8.lock"
...
def write_v8_birth() -> dict[str, Any]:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.V8.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "note": "Scorer-only cycle. Neural unchanged from candidate v7.",
    }
    if V8_BIRTH.exists():
        raise RuntimeError("v8 birth exists")
    V8_BIRTH.write_text(json.dumps(birth, indent=2, default=str) + "\n", encoding="utf-8")
    return {"ok": True, "sha": _sha_file(V8_BIRTH)}


def write_v8_math_audit() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="v8audit_") as tmp:
        ag = make_cortex(Path(tmp) / "s")
    ...
    out = {
        "version": "TM.0.23.CORTEX.V8.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if V8_MATH.exists():
        raise RuntimeError("v8 math audit exists")
    V8_MATH.write_text(json.dumps(out, indent=2) + "\n", encoding="utf-8")
    return out


def write_candidate_v8() -> dict[str, Any]:
    if not V8_BIRTH.exists() or not V8_MATH.exists():
        raise RuntimeError("birth + math audit required")
    if not json.loads(V8_MATH.read_text(encoding="utf-8")).get("ok"):
        raise RuntimeError("math audit failed")
    if CANDIDATE_V8.exists():
        raise RuntimeError("candidate.v8 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V8",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v7_sha": _sha_file(CANDIDATE_V7),
        "v8_birth_sha": _sha_file(V8_BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        "diagnosis_v7_sha": _sha_file(DIAG),
        "stat_contract_sha": _sha_file(STAT),
        "math_audit_sha": _sha_file(V8_MATH),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v8 reuses v7 neural under the v8 scorer contract. Must re-earn D1–D2. No DEVELOP yet.",
    }
    CANDIDATE_V8.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v8_sha": _sha_file(CANDIDATE_V8)}


def run_boundary_v8(*, write_lock: bool = False) -> dict[str, Any]:
    if not CANDIDATE_V8.exists():
        raise RuntimeError("missing candidate v8")
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V8",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v8.lock",
        "candidate_sha": _sha_file(CANDIDATE_V8),
        ...
        "env": torch_env(),
        "note": "v8 required greens are C4 + population C5/C6. C7 single-life bar not used.",
    }
    if write_lock:
        if MACT_V8.exists():
            raise RuntimeError("boundary v8 lock exists")
        MACT_V8.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        lines = ["# TM.0.23.CORTEX M_act boundary (v8)", "", f"**required_green:** `{summary['all_required_green']}`", ""]
...
def reveal_v8_eval() -> dict[str, Any]:
    if not CANDIDATE_V8.exists() or not MACT_V8.exists():
        raise RuntimeError("candidate v8 + boundary required")
    if not json.loads(MACT_V8.read_text(encoding="utf-8")).get("all_required_green"):
        raise RuntimeError("v8 population contract not green")
    if not _git_clean():
        raise RuntimeError("working tree dirty — refuse reveal")
    git_sha = _git_head()
    if V8_REVEAL.exists():
        raise RuntimeError("v8 reveal exists")
    sealed = json.loads(V8_SEALED.read_text(encoding="utf-8"))
    commitment = hashlib.sha256(bytes.fromhex(sealed["seed_hex"]) + bytes.fromhex(sealed["salt_hex"])).hexdigest()
    prereg = json.loads(V8_PREREG.read_text(encoding="utf-8"))
    if commitment != prereg["eval_seed_commitment"]:
        raise RuntimeError("commitment mismatch")
    reveal = {
        "version": "TM.0.23.CORTEX.V8.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v8_sha": _sha_file(CANDIDATE_V8),
        "candidate_git_sha": git_sha,
        "mact_boundary_v8_sha": _sha_file(MACT_V8),
        "note": "Reveal only after candidate commit is on a clean HEAD.",
    }
    V8_REVEAL.write_text(json.dumps(reveal, indent=2) + "\n", encoding="utf-8")
    V8_GATE_PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V8.GATE.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_seed_commitment": commitment,
                "reveal_sha": _sha_file(V8_REVEAL),
                "candidate_v8_sha": _sha_file(CANDIDATE_V8),
                "candidate_git_sha": git_sha,
    ...
    return {"ok": True, "reveal_sha": _sha_file(V8_REVEAL), "candidate_git_sha": git_sha}


def run_v8_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    if not V8_REVEAL.exists():
        raise RuntimeError("reveal first")
    if DEV_V8.exists():
        raise RuntimeError("DEVELOP.v8 exists before gate — refuse")
    cand = json.loads(CANDIDATE_V8.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(V8_REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_v8_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.V8.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Narrow D1–D2 under v8 stat contract. Full D0–D12 stays closed unless ≥13/16.",
    }
    if write_lock:
        if V8_GATE_LOCK.exists():
            raise RuntimeError("gate lock exists")
        V8_GATE_LOCK.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        clear = battery["sensorimotor_association_gate_clear"]
        V8_GATE_MD.write_text(
            f"# TM.0.23.CORTEX v8 gate\n\n**sensorimotor_association_gate_clear:** `{clear}`\n**n_pair_clear:** `{battery['n_pair_clear']}/16`\n\n",
            encoding="utf-8",
        )
        if not clear:
            V8_FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.V8.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
                        "gate_sha": _sha_file(V8_GATE_LOCK),
                        "n_pair_clear": battery["n_pair_clear"],
                        "refuse": ["DEVELOP.v8", "edit-and-rescore on revealed v8 worlds", "full D0–D12"],
                    },
                    indent=2,
                )
                + "\n",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    if args.write_birth:
        print(json.dumps(write_v8_birth(), indent=2, default=str))
    elif args.math_audit:
        print(json.dumps(write_v8_math_audit(), indent=2, default=str))
    elif args.write_candidate:
        print(json.dumps(write_candidate_v8(), indent=2))
    elif args.mact_boundary:
        print(json.dumps(run_boundary_v8(write_lock=args.write_lock), indent=2, default=str))
    elif args.reveal_gate:
        print(json.dumps(reveal_v8_eval(), indent=2))
    elif args.gate:
        print(json.dumps(run_v8_gate(device=args.device, write_lock=args.write_lock), indent=2, default=str))
    else:
        ap.print_help()
