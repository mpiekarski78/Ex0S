"""Freeze v13 contract and commitment. No neural edits. No candidate SHA."""
...
STAT_MD = DOCS / "cortex_v13_stat_contract.md"
STAT_LOCK = DOCS / "cortex_v13_stat_contract.lock"
GATE_MD = DOCS / "cortex_v13_gate_contract.md"
AMEND_MD = DOCS / "cortex_v13_architecture_amendment.md"
AMEND_LOCK = DOCS / "cortex_v13_architecture_amendment.lock"
DIAG = DOCS / "cortex_diagnosis.v12.lock"
NEURAL = REPO_ROOT / "three_memory" / "neural_cortex.py"
CAND12 = DOCS / "cortex.candidate.v12.lock"
GATE_PY = REPO_ROOT / "experiments" / "cortex_v13_gate.py"
WORLDS_PY = REPO_ROOT / "experiments" / "cortex_v13_worlds.py"
SCORERS_PY = REPO_ROOT / "experiments" / "cortex_v11_scorers.py"
RUNNER = DOCS / "cortex_v13_gate.runner.lock"
PREREG = DOCS / "cortex_v13.prereg.lock"
SEALED = DOCS / "cortex_v13_eval_secrets.sealed.json"
...
def freeze_v13_apparatus() -> dict[str, Any]:
    if not DIAG.exists():
        raise RuntimeError("freeze v12 diagnosis first")
    if STAT_LOCK.exists() or PREREG.exists():
        raise RuntimeError("v13 apparatus already frozen")
    cand = json.loads(CAND12.read_text(encoding="utf-8"))
    if _sha(NEURAL) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted — freeze apparatus before neural v13 edits")
    prior = []
    for name in (
    ...
    stat = {
        "version": "TM.0.23.CORTEX.V13.STAT.CONTRACT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "diagnosis_v12_sha": _sha(DIAG),
        "neural_sha_at_freeze": _sha(NEURAL),
        "neural_change_authorized": True,
        "authorized_law": "slow_advantage_baseline",
        ...
        "refuse": [
            "lower holds>=5",
            "raise CONFLICT_HOLD_BIAS",
            "edit-and-rescore v12",
    ...
    AMEND_LOCK.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V13.ARCHITECTURE.AMENDMENT",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "diagnosis_v12_sha": _sha(DIAG),
                ...
                "neural_sha_at_freeze": _sha(NEURAL),
                "changes_authorized": ["slow_advantage_baseline"],
            },
            indent=2,
        )
        + "\n",
    ...
    RUNNER.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V13.GATE.RUNNER",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                "eval_revealed": False,
                ...
                "gate_module": "experiments.cortex_v13_gate",
    ...
    if commitment in prior:
        raise RuntimeError("v13 commitment collision")
    SEALED.write_text(
        json.dumps({"version": "TM.0.23.CORTEX.V13.EVAL.SEALED", "seed_hex": seed_b.hex(), "salt_hex": salt_b.hex()}, indent=2)
        + "\n"
    )
    PREREG.write_text(
        json.dumps(
            {
                "version": "TM.0.23.CORTEX.V13.PREREG",
                "product": "0.0.004",
                "earned_next": False,
                "ex0s": None,
                ...
                "note": "Narrow v13 D1–D2. Neural slow baseline authorized after this freeze. Not DEVELOP.",
            },
            indent=2,
        )
        + "\n",
...
if __name__ == "__main__":
    print(json.dumps(freeze_v13_apparatus(), indent=2))
