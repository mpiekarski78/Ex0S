"""TM.0.23.CORTEX.FULLDEV.R3 — reveal and score D0–D12."""
...
from experiments.cortex_fulldev_r3 import run_fulldev_r3_battery
...
REVEAL = DOCS / "cortex_fulldev_r3_eval_reveal.lock"
RUNNER = DOCS / "cortex_fulldev_r3.runner.lock"
RESULT = DOCS / "cortex_fulldev_r3.lock"
RESULT_MD = DOCS / "tm023cortex_fulldev_r3_results.md"
FAIL = DOCS / "cortex_fulldev_r3.failure.lock"
DEV_V13 = DOCS / "cortex_development.v13.lock"
CANDIDATE_GIT = "b3563d872233dd0547128f68bed544e2fd6347e5"
...
def reveal_fulldev_r3() -> dict[str, Any]:
    if not PREREG.exists() or not RUNNER.exists() or not CANDIDATE_V18.exists():
        raise RuntimeError("FULLDEV.R3 apparatus + candidate v18 required")
    if DEV_V13.exists():
        raise RuntimeError("DEVELOP.v13 exists — refuse")
    if not _git_clean():
        raise RuntimeError("working tree dirty — refuse reveal")
    if REVEAL.exists():
        raise RuntimeError("FULLDEV.R3 reveal exists")
    git_sha = _git_head()
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.FULLDEV.R3.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def run_fulldev_r3(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    ...
    battery = run_fulldev_r3_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.FULLDEV.R3.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Full D0–D12 on TM023.FULL.R3. worlds. Does not stamp 0.0.005.",
    }
    if write_lock:
        if RESULT.exists():
            raise RuntimeError("FULLDEV.R3 result exists")
        RESULT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        RESULT_MD.write_text(
            "# TM.0.23.CORTEX FULLDEV.R3\n\n"
        ...
        if not battery["development_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.FULLDEV.R3.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
                        ...
                        "refuse": [
                            "edit-and-rescore on revealed FULLDEV.R3 worlds",
                            "stamp earned_next",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    if args.reveal:
        print(json.dumps(reveal_fulldev_r3(), indent=2))
    elif args.score:
        print(json.dumps(run_fulldev_r3(device=args.device, write_lock=args.write_lock), indent=2, default=str))
    else:
        ap.print_help()
