"""TM.0.23.CORTEX.D5.R2 — birth/candidate with habituation familiarity."""
...
from experiments.cortex_d5_r2_gate import run_d5_r2_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.v19.lock"
STAT = DOCS / "cortex_d5_r2_stat_contract.lock"
AMEND = DOCS / "cortex_d5_r2_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d5_r2_birth.lock"
MATH = DOCS / "cortex_d5_r2_math_audit.lock"
CANDIDATE_V20 = DOCS / "cortex.candidate.v20.lock"
CANDIDATE_LIVE = DOCS / "cortex.candidate.lock"
CANDIDATE_V19 = DOCS / "cortex.candidate.v19.lock"
PREREG = DOCS / "cortex_d5_r2.prereg.lock"
SEALED = DOCS / "cortex_d5_r2_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d5_r2_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d5_r2_gate.lock"
GATE_MD = DOCS / "tm023cortex_d5_r2_gate_results.md"
FAIL = DOCS / "cortex_d5_r2_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v20.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v20_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d5r2birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D5.R2.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_v19_sha": _sha_file(DIAG),
        ...
        "prior_v19_neural_sha": json.loads(CANDIDATE_V19.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D5.R2 habituation familiarity. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D5.R2 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("familiarity_abs_16", FAMILIARITY_ABS == 16.0)
    add("no_lifetime_max_novelty", "FAMILIARITY_RATIO * float(mx)" not in src)
    add("neural_changed_from_v19", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V19.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D5.R2.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V20.exists():
        raise RuntimeError("candidate.v20 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V20",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v19_sha": _sha_file(CANDIDATE_V19),
        "d5_r2_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v20 / D5.R2. Must re-earn C4/C5/C6 then D5. No FULLDEV reopen yet.",
    }
    CANDIDATE_V20.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v20_sha": _sha_file(CANDIDATE_V20)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V20.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V20",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v20.lock",
        "candidate_sha": _sha_file(CANDIDATE_V20),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v20 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v20)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D5.R2.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v20_sha": _sha_file(CANDIDATE_V20),
        "candidate_git_sha": git_sha,
        "mact_boundary_v20_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V20.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d5_r2_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D5.R2.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if write_lock:
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D5.R2.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
