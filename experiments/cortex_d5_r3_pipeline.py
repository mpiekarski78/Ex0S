"""TM.0.23.CORTEX.D5.R3 — birth/candidate with neophobia on unfamiliar phrase."""
...
from experiments.cortex_d5_r3_gate import run_d5_r3_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.fulldev_r6.lock"
STAT = DOCS / "cortex_d5_r3_stat_contract.lock"
AMEND = DOCS / "cortex_d5_r3_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d5_r3_birth.lock"
MATH = DOCS / "cortex_d5_r3_math_audit.lock"
...
PREREG = DOCS / "cortex_d5_r3.prereg.lock"
SEALED = DOCS / "cortex_d5_r3_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d5_r3_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d5_r3_gate.lock"
GATE_MD = DOCS / "tm023cortex_d5_r3_gate_results.md"
FAIL = DOCS / "cortex_d5_r3_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v26.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v26_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d5r3birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D5.R3.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_fulldev_r6_sha": _sha_file(DIAG),
        ...
        "prior_v25_neural_sha": json.loads(CANDIDATE_V25.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D5.R3 neophobia on unfamiliar phrase. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D5.R3 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("phrase_final_stop_kept", "_phrase" in src and "phrase_target" in src)
    add("neophobia_blocks_unfamiliar_phrase", "FAMILIARITY_ABS" in src and "unfamiliar" in src)
    add("no_lifetime_max_novelty", "FAMILIARITY_RATIO * float(mx)" not in src)
    add("neural_changed_from_v25", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V25.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D5.R3.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V26.exists():
        raise RuntimeError("candidate.v26 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V26",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v25_sha": _sha_file(CANDIDATE_V25),
        "d5_r3_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v26 / D5.R3. Must re-earn C4/C5/C6 then D5. No FULLDEV reopen yet.",
    }
    CANDIDATE_V26.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v26_sha": _sha_file(CANDIDATE_V26)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V26.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V26",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v26.lock",
        "candidate_sha": _sha_file(CANDIDATE_V26),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v26 exists")
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D5.R3.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v26_sha": _sha_file(CANDIDATE_V26),
        "candidate_git_sha": git_sha,
        "mact_boundary_v26_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V26.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d5_r3_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D5.R3.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
    ...
    if write_lock:
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D5.R3.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
