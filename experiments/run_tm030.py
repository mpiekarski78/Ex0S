"""TM.0.3.0: a life. Find/commit/use in a free life; probe after ρ reset with W gone.

A split credit. B shared return.
Not the scripted probe→unmount→probe train of TM.0.1.2 / TM.0.2.0.
"""
...
from experiments.run_tm012 import (
    SWAP_RED,
    WIKI_RED,
    _has_field,
    wiki_notes as messy_wiki_notes,
)
...
def _run_dir() -> Path:
    ...
    d = REPO_ROOT / "runs" / f"{stamp}_tm030"
    d.mkdir(parents=True, exist_ok=True)
    return d


def wiki_notes(*, include_red: bool = False, include_green: bool = False) -> list[tuple[str, dict[str, Any]]]:
    return messy_wiki_notes(include_red=include_red, include_green=include_green)
...
def classify_common(m: dict[str, Any]) -> tuple[str, str] | None:
    ...
    if m["trained_force_use"]:
        return "Fail", "Use clamped to rescue the life."
...
def classify_a(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if m["untrained_probe"]["correct"] or m["untrained_probe"]["action_name"] == "use_key":
        return "Fail", "Untrained already used the key after a free life."
    if not m["red_after_reset"]["correct"]:
        return "Fail", "Free red life did not leave a usable S after ρ reset / W gone."
    if not m["green_after_reset"]["correct"]:
        return "Fail", "Held-out green free life failed after reset."
    ...
    if not m["red_live"]["found_action2"]:
        return "Fail", "Free red life never committed the messy unread page."
    tag = m.get("red_tag", "")
    if _has_field(tag, "loc") or _has_field(tag, "door") or _has_field(tag, "here"):
        return "Fail", "Red S used an exact place-name query."
    return (
        "Store-works",
        "Free life found/committed messy W; after ρ reset and W gone, probe uses S; cortex frozen; held-out green wait.",
    )
...
def classify_b(m: dict[str, Any]) -> tuple[str, str]:
    ...
    if not m["red_after_reset"]["correct"]:
        return "Fail", "Shared return did not solve red after a free life."
    if not m["green_after_reset"]["correct"]:
    ...
    return (
        "Store-works",
        "Shared return on a free life; cortex frozen; W gone after reset; held-out green wait.",
    )
...
def _train(
    ...
    *,
    ...
    for ep in range(n):
        ...
        ag.world = None
        ag.reset_rho()
        # Probe records use traces (and search over S).
...
def run_arm(
    *,
    ...
    un_a, untrained_live, untrained_probe, _ = _life_then_probe(
        make,
        s_un,
        w_red,
        policy,
...
def run_tm030(seed: int = 12345, n_train: int = 500, max_steps: int = 32) -> dict[str, Any]:
    run_dir = _run_dir()
    w_red = run_dir / "W_red"
    ...
    (run_dir / "summary.md").write_text(
        f"""# TM.0.3.0 A free life vs B shared return

| Arm | Classification | Train last 50 |
|-----|----------------|---------------|
| A split free life | **{a['classification']}** | {a['train_return_last50']:.2f} |
...
| Red life found messy page | {a['red_live']['found_action2']} | {b['red_live']['found_action2']} |
| n_forced red/green | {a['red_live']['n_forced']} / {a['green_live']['n_forced']} | {b['red_live']['n_forced']} / {b['green_live']['n_forced']} |
...
def main() -> None:
    p = argparse.ArgumentParser(description="TM.0.3.0 free life; split vs shared return")
    p.add_argument("--seed", type=int, default=12345)
    p.add_argument("--n-train", type=int, default=500)
    p.add_argument("--max-steps", type=int, default=32)
    args = p.parse_args()
    m = run_tm030(seed=args.seed, n_train=args.n_train, max_steps=args.max_steps)
