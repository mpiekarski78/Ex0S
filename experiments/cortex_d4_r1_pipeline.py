"""TM.0.23.CORTEX.D4.R1 — birth/candidate with S-write recency retrieval."""
...
from experiments.cortex_d4_r1_gate import run_d4_r1_gate_battery
...
DIAG = DOCS / "cortex_diagnosis.fulldev_r2.lock"
STAT = DOCS / "cortex_d4_r1_stat_contract.lock"
AMEND = DOCS / "cortex_d4_r1_architecture_amendment.lock"
BIRTH = DOCS / "cortex_d4_r1_birth.lock"
MATH = DOCS / "cortex_d4_r1_math_audit.lock"
CANDIDATE_V17 = DOCS / "cortex.candidate.v17.lock"
CANDIDATE_LIVE = DOCS / "cortex.candidate.lock"
CANDIDATE_V16 = DOCS / "cortex.candidate.v16.lock"
PREREG = DOCS / "cortex_d4_r1.prereg.lock"
SEALED = DOCS / "cortex_d4_r1_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_d4_r1_eval_reveal.lock"
GATE_LOCK = DOCS / "cortex_d4_r1_gate.lock"
GATE_MD = DOCS / "tm023cortex_d4_r1_gate_results.md"
FAIL = DOCS / "cortex_d4_r1_gate.failure.lock"
MACT = DOCS / "cortex_mact_boundary.v17.lock"
MACT_MD = DOCS / "tm023cortex_mact_boundary_v17_results.md"
...
def write_birth() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="d4r1birth_") as tmp:
    ...
    birth = {
        "version": "TM.0.23.CORTEX.D4.R1.BIRTH",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "diagnosis_fulldev_r2_sha": _sha_file(DIAG),
        ...
        "prior_v16_neural_sha": json.loads(CANDIDATE_V16.read_text(encoding="utf-8"))["neural_cortex_sha"],
        ...
        "note": "D4.R1 S-write recency retrieval. No FULLDEV reopen yet.",
    }
    if BIRTH.exists():
        raise RuntimeError("D4.R1 birth exists")
...
def write_math_audit() -> dict[str, Any]:
    ...
    add("s_write_recency_slot0", "buf[0] = v" in src)
    add("memory_on_write_callback", "self.on_write" in mem and "on_write(rec)" in mem)
    add("restore_does_not_fire_hook", "self.on_write = None" in mem)
    add("neural_changed_from_v16", _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V16.read_text(encoding="utf-8"))["neural_cortex_sha"])
    out = {
        "version": "TM.0.23.CORTEX.D4.R1.MATH.AUDIT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
...
def write_candidate() -> dict[str, Any]:
    ...
    if CANDIDATE_V17.exists():
        raise RuntimeError("candidate.v17 exists")
    cand = {
        "version": "TM.0.23.CORTEX.CANDIDATE.V17",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "factory": "experiments.run_tm023cortex.make_cortex",
        "supersedes_v16_sha": _sha_file(CANDIDATE_V16),
        "d4_r1_birth_sha": _sha_file(BIRTH),
        "architecture_amendment_sha": _sha_file(AMEND),
        ...
        "env": torch_env(),
        "genome": GenomeConfig().to_dict(),
        "note": "Candidate v17 / D4.R1. Must re-earn C4/C5/C6 then D4. No FULLDEV reopen yet.",
    }
    CANDIDATE_V17.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    CANDIDATE_LIVE.write_text(json.dumps(cand, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "candidate_v17_sha": _sha_file(CANDIDATE_V17)}


def run_boundary(*, write_lock: bool = False) -> dict[str, Any]:
    if _sha_file(NEURAL_PY) != json.loads(CANDIDATE_V17.read_text(encoding="utf-8"))["neural_cortex_sha"]:
    ...
    summary = {
        "version": "TM.0.23.CORTEX.MACT.BOUNDARY.RESULT.V17",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        "candidate": "docs/cortex.candidate.v17.lock",
        "candidate_sha": _sha_file(CANDIDATE_V17),
    ...
    if write_lock:
        if MACT.exists():
            raise RuntimeError("boundary v17 exists")
        MACT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        MACT_MD.write_text(
            "# TM.0.23.CORTEX M_act boundary (v17)\n\n"
...
def reveal() -> dict[str, Any]:
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.D4.R1.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "candidate_v17_sha": _sha_file(CANDIDATE_V17),
        "candidate_git_sha": git_sha,
        "mact_boundary_v17_sha": _sha_file(MACT),
    }
...
def run_gate(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    cand = json.loads(CANDIDATE_V17.read_text(encoding="utf-8"))
    if _sha_file(NEURAL_PY) != cand["neural_cortex_sha"]:
        raise RuntimeError("neural drifted")
    reveal = json.loads(REVEAL.read_text(encoding="utf-8"))
    dev = device or ("cuda" if torch.cuda.is_available() else "cpu")
    battery = run_d4_r1_gate_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.D4.R1.GATE.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Narrow D4. Full D0–D12 stays closed unless ≥13/16.",
    }
    if write_lock:
        if GATE_LOCK.exists():
        ...
        if not battery["relation_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.D4.R1.GATE.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
