"""Derive v12 gate lives from a revealed 256-bit eval seed."""
...
def _ints(seed_hex: str, pair_id: int, role: str) -> list[int]:
    raw = hashlib.sha256(bytes.fromhex(seed_hex) + b"TM023.V12." + role.encode() + pair_id.to_bytes(2, "big")).digest()
...
def sealed_pair_seeds(pair_id: int, seed_hex: str) -> tuple[LifeSeeds, LifeSeeds]:
    if pair_id < 0 or pair_id > 15:
        raise ValueError("v12 gate uses pair_id 0..15")
