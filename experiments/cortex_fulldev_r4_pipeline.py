"""TM.0.23.CORTEX.FULLDEV.R4 — reveal and score D0–D12."""
...
from experiments.cortex_fulldev_r4 import run_fulldev_r4_battery
...
PREREG = DOCS / "cortex_fulldev_r4.prereg.lock"
SEALED = DOCS / "cortex_fulldev_r4_eval_secrets.sealed.json"
REVEAL = DOCS / "cortex_fulldev_r4_eval_reveal.lock"
RUNNER = DOCS / "cortex_fulldev_r4.runner.lock"
RESULT = DOCS / "cortex_fulldev_r4.lock"
RESULT_MD = DOCS / "tm023cortex_fulldev_r4_results.md"
FAIL = DOCS / "cortex_fulldev_r4.failure.lock"
DEV_V13 = DOCS / "cortex_development.v13.lock"
CANDIDATE_GIT = "569ffe909c8a80bc14b10414ea021eef3440d451"
...
def reveal_fulldev_r4() -> dict[str, Any]:
    if not PREREG.exists() or not RUNNER.exists() or not CANDIDATE_V20.exists():
        raise RuntimeError("FULLDEV.R4 apparatus + candidate v20 required")
    if DEV_V13.exists():
        raise RuntimeError("DEVELOP.v13 exists — refuse")
    if not _git_clean():
        raise RuntimeError("working tree dirty — refuse reveal")
    if REVEAL.exists():
        raise RuntimeError("FULLDEV.R4 reveal exists")
    git_sha = _git_head()
    ...
    reveal = {
        "version": "TM.0.23.CORTEX.FULLDEV.R4.EVAL.REVEAL",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "note": "Reveal only after apparatus commit is on a clean HEAD. Worlds ≠ FULL.R1–R2, D3.R*, D4.R*, pair_seeds.",
    }
    REVEAL.write_text(json.dumps(reveal, indent=2) + "\n", encoding="utf-8")
    return {"ok": True, "reveal_sha": _sha_file(REVEAL), "apparatus_git_sha": git_sha}


def run_fulldev_r4(*, device: str | None = None, write_lock: bool = False) -> dict[str, Any]:
    ...
    battery = run_fulldev_r4_battery(seed_hex=reveal["seed_hex"], n_pairs=16, device=dev)
    summary = {
        "version": "TM.0.23.CORTEX.FULLDEV.R4.RESULT",
        "product": "0.0.004",
        "earned_next": False,
        "ex0s": None,
        ...
        "env": torch_env(),
        "device": dev,
        "note": "Full D0–D12 on TM023.FULL.R4. worlds. Does not stamp 0.0.005.",
    }
    if write_lock:
        if RESULT.exists():
            raise RuntimeError("FULLDEV.R4 result exists")
        RESULT.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
        RESULT_MD.write_text(
            "# TM.0.23.CORTEX FULLDEV.R4\n\n"
        ...
        if not battery["development_gate_clear"]:
            FAIL.write_text(
                json.dumps(
                    {
                        "version": "TM.0.23.CORTEX.FULLDEV.R4.FAILURE",
                        "product": "0.0.004",
                        "earned_next": False,
                        "ex0s": None,
                        ...
                        "refuse": [
                            "edit-and-rescore on revealed FULLDEV.R4 worlds",
                            "stamp earned_next",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    if args.reveal:
        print(json.dumps(reveal_fulldev_r4(), indent=2))
    elif args.score:
        print(json.dumps(run_fulldev_r4(device=args.device, write_lock=args.write_lock), indent=2, default=str))
    else:
        ap.print_help()
