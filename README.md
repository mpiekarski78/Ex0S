...
Honest status after v22: the **split is real on a toy**. Not a general learner. Still genome: two-key name menus `{action, do}` / `{door, here}`, generic `logits[int] += 3.0`, exact match, frozen commit-on-hit, frozen complete-vs-stub / newest-`when=` among W hits, split credit, four discrete acts. Tiny LSTM still needs a taught tool grammar for English. W is a handful of `.tag` files, not Wikipedia. No code, no cameras.
...
experiments/      # run_v0 … run_v22, train_prior
docs/             # protocol, comparison, conclusion, v1–v22 results
tests/
runs/             # gitignored
