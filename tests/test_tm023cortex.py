...
def test_birth_and_candidate_frozen() -> None:
    ...
    if cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V30":
        v30_birth = json.loads((REPO_ROOT / "docs" / "cortex_v30_birth.lock").read_text(encoding="utf-8"))
        assert v30_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V29":
        v29_birth = json.loads((REPO_ROOT / "docs" / "cortex_v29_birth.lock").read_text(encoding="utf-8"))
        assert v29_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V28":
        ...
        v24_birth = json.loads((REPO_ROOT / "docs" / "cortex_d7_r1_birth.lock").read_text(encoding="utf-8"))
        assert v24_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V23":
        v23_birth = json.loads((REPO_ROOT / "docs" / "cortex_d6_r3_birth.lock").read_text(encoding="utf-8"))
        assert v23_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V22":
        v22_birth = json.loads((REPO_ROOT / "docs" / "cortex_d6_r2_birth.lock").read_text(encoding="utf-8"))
        assert v22_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V21":
        v21_birth = json.loads((REPO_ROOT / "docs" / "cortex_d6_r1_birth.lock").read_text(encoding="utf-8"))
        assert v21_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V20":
        v20_birth = json.loads((REPO_ROOT / "docs" / "cortex_d5_r2_birth.lock").read_text(encoding="utf-8"))
        assert v20_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V19":
        v19_birth = json.loads((REPO_ROOT / "docs" / "cortex_d5_r1_birth.lock").read_text(encoding="utf-8"))
        assert v19_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V18":
        v18_birth = json.loads((REPO_ROOT / "docs" / "cortex_d4_r2_birth.lock").read_text(encoding="utf-8"))
        assert v18_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V17":
        v17_birth = json.loads((REPO_ROOT / "docs" / "cortex_d4_r1_birth.lock").read_text(encoding="utf-8"))
        assert v17_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V16":
        v16_birth = json.loads((REPO_ROOT / "docs" / "cortex_d3_r3_birth.lock").read_text(encoding="utf-8"))
        assert v16_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V15":
        v15_birth = json.loads((REPO_ROOT / "docs" / "cortex_d3_r2_birth.lock").read_text(encoding="utf-8"))
        assert v15_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V14":
        ...
        v11_birth = json.loads((REPO_ROOT / "docs" / "cortex_v11_birth.lock").read_text(encoding="utf-8"))
        assert v11_birth["learning_law_ok"] is True
    elif cand.get("version") == "TM.0.23.CORTEX.CANDIDATE.V10":
...
def test_diag_and_v4_gate() -> None:
    ...
    v29 = REPO_ROOT / "docs" / "cortex.candidate.v29.lock"
    ...
    v22 = REPO_ROOT / "docs" / "cortex.candidate.v22.lock"
    v21 = REPO_ROOT / "docs" / "cortex.candidate.v21.lock"
    v20 = REPO_ROOT / "docs" / "cortex.candidate.v20.lock"
    v19 = REPO_ROOT / "docs" / "cortex.candidate.v19.lock"
    v18 = REPO_ROOT / "docs" / "cortex.candidate.v18.lock"
    v17 = REPO_ROOT / "docs" / "cortex.candidate.v17.lock"
    v16 = REPO_ROOT / "docs" / "cortex.candidate.v16.lock"
    v15 = REPO_ROOT / "docs" / "cortex.candidate.v15.lock"
    ...
    if v30.exists():
        ...
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V29"
    elif v28.exists():
        ...
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V23"
    elif v22.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V22"
    elif v21.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V21"
    elif v20.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V20"
    elif v19.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V19"
    elif v18.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V18"
    elif v17.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V17"
    elif v16.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V16"
    elif v15.exists():
        assert live["version"] == "TM.0.23.CORTEX.CANDIDATE.V15"
    elif v14.exists():
...
def test_verify_v11_gate_cli() -> None:
    ...
    if v["sensorimotor_association_gate_clear"]:
        ...
        gate = json.loads((REPO_ROOT / "docs" / "cortex_v11_gate.lock").read_text(encoding="utf-8"))
        assert gate["battery"]["n_pair_clear"] == 6
        assert gate["battery"]["d2_conflict"] == "swapped_press_harm"
        assert gate["battery"]["stage_ok_counts"]["D1"] == 32
        iso = json.loads((REPO_ROOT / "docs" / "cortex_v12.isolation.lock").read_text(encoding="utf-8"))
        assert "DEVELOP.v11 on any worlds" in iso["refuse"]


def test_v12_candidate_boundary_pending_gate() -> None:
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v12.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    ...
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_v12.prereg.lock").read_text(encoding="utf-8"))
    v11p = json.loads((REPO_ROOT / "docs" / "cortex_v11.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != v11p["eval_seed_commitment"]
    assert prereg["authorized_law"] == "surprise_conflict_hold"
    assert prereg["d2_conflict"] == "swapped_press_harm"
    gate_p = REPO_ROOT / "docs" / "cortex_v12_gate.lock"
    if gate_p.exists():
        gate = json.loads(gate_p.read_text(encoding="utf-8"))
        if gate["sensorimotor_association_gate_clear"]:
            assert gate["battery"]["n_pair_clear"] >= 13
        else:
            assert not (REPO_ROOT / "docs" / "cortex_development.v12.lock").exists()
...
def test_verify_v12_gate_cli() -> None:
    from experiments.run_tm023cortex import verify_v12_gate

    v = verify_v12_gate()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v12.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_v12_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
...
def test_verify_v13_gate_cli() -> None:
    from experiments.run_tm023cortex import verify_v13_gate

    v = verify_v13_gate()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v13.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_v13_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
...
def test_v30_candidate_boundary_pending_gate() -> None:
    ...
    iso = json.loads((REPO_ROOT / "docs" / "cortex_v30.isolation.lock").read_text(encoding="utf-8"))
    cells = json.loads((REPO_ROOT / "docs" / "lineage_motorpersist.prereg.lock").read_text(encoding="utf-8"))
    assert amend["n"] == 64
    assert prereg["n"] == 64
    assert prereg["authorized_law"] == "scalar_persistence_on_zero_input_motor_tick"
    assert "increase_n" in prereg["refuse"]
    assert "increase n" in iso["refuse"]
    assert cells["p_grid"][-1] == 0.95
    assert cells["lineage_after_v30_pass"] is False
    assert not (REPO_ROOT / "docs" / "cortex_fulldev_r7.prereg.lock").exists()
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v30.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V30"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v29 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v29.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v29["neural_cortex_sha"]
    assert cand["genome"]["n"] == 64


def test_v29_candidate_boundary_pending_gate() -> None:
    amend = json.loads((REPO_ROOT / "docs" / "cortex_v29_architecture_amendment.lock").read_text(encoding="utf-8"))
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_v29.prereg.lock").read_text(encoding="utf-8"))
    iso = json.loads((REPO_ROOT / "docs" / "cortex_v29.isolation.lock").read_text(encoding="utf-8"))
    cells = json.loads((REPO_ROOT / "docs" / "lineage_actorcredit.prereg.lock").read_text(encoding="utf-8"))
    assert amend["n"] == 64
    assert prereg["n"] == 64
    assert prereg["authorized_law"] == "action_owned_delayed_credit"
    assert "increase_n" in prereg["refuse"]
    assert "increase n" in iso["refuse"]
    assert cells["cells"][-1] == "A11"
    assert cells["reachability_only_if_cells_pass"] is True
    assert not (REPO_ROOT / "docs" / "cortex_fulldev_r7.prereg.lock").exists()
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v29.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V29"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v28 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v28.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v28["neural_cortex_sha"]
    assert cand["genome"]["n"] == 64
...
def test_v27_candidate_boundary_pending_gate() -> None:
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v27.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V27"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v26 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v26.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v26["neural_cortex_sha"]
    mact_p = REPO_ROOT / "docs" / "cortex_mact_boundary.v27.lock"
    if mact_p.exists():
        mact = json.loads(mact_p.read_text(encoding="utf-8"))
        required = {"C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"}
        if mact.get("all_required_green"):
            assert required.issubset({c["id"] for c in mact["controls"] if c.get("ok")})
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_v27.prereg.lock").read_text(encoding="utf-8"))
    d5p = json.loads((REPO_ROOT / "docs" / "cortex_d5_r3.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != d5p["eval_seed_commitment"]
...
def test_v25_candidate_boundary_pending_gate() -> None:
    ...
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V25"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v24 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v24.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v24["neural_cortex_sha"]
    mact_p = REPO_ROOT / "docs" / "cortex_mact_boundary.v25.lock"
    if mact_p.exists():
        mact = json.loads(mact_p.read_text(encoding="utf-8"))
        required = {"C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"}
        if mact.get("all_required_green"):
            assert required.issubset({c["id"] for c in mact["controls"] if c.get("ok")})
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_d7_r2.prereg.lock").read_text(encoding="utf-8"))
    r1p = json.loads((REPO_ROOT / "docs" / "cortex_d7_r1.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != r1p["eval_seed_commitment"]
    assert prereg["authorized_law"] == "phrase_final_stop"
    assert prereg["domain"] == "TM023.D7.R2."


def test_v24_candidate_boundary_pending_gate() -> None:
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v24.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V24"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v23 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v23.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v23["neural_cortex_sha"]
    mact_p = REPO_ROOT / "docs" / "cortex_mact_boundary.v24.lock"
    if mact_p.exists():
        mact = json.loads(mact_p.read_text(encoding="utf-8"))
        required = {"C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"}
        if mact.get("all_required_green"):
            assert required.issubset({c["id"] for c in mact["controls"] if c.get("ok")})
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_d7_r1.prereg.lock").read_text(encoding="utf-8"))
    r5p = json.loads((REPO_ROOT / "docs" / "cortex_fulldev_r5.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != r5p["eval_seed_commitment"]
    assert prereg["authorized_law"] == "utterance_persistence"
    assert prereg["domain"] == "TM023.D7.R1."


def test_v16_candidate_boundary_pending_gate() -> None:
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v16.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V16"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v15 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v15.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v15["neural_cortex_sha"]
    mact_p = REPO_ROOT / "docs" / "cortex_mact_boundary.v16.lock"
    if mact_p.exists():
        mact = json.loads(mact_p.read_text(encoding="utf-8"))
        required = {"C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"}
        if mact.get("all_required_green"):
    ...
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_d3_r3.prereg.lock").read_text(encoding="utf-8"))
    r2p = json.loads((REPO_ROOT / "docs" / "cortex_d3_r2.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != r2p["eval_seed_commitment"]
    assert prereg["authorized_law"] == "current_observe_body_adv_equal_evidence"
    assert prereg["domain"] == "TM023.D3.R3."


def test_v15_candidate_boundary_pending_gate() -> None:
    cand_p = REPO_ROOT / "docs" / "cortex.candidate.v15.lock"
    if not cand_p.exists():
        return
    cand = json.loads(cand_p.read_text(encoding="utf-8"))
    assert cand["version"] == "TM.0.23.CORTEX.CANDIDATE.V15"
    assert cand["earned_next"] is False
    assert cand["ex0s"] is None
    v14 = json.loads((REPO_ROOT / "docs" / "cortex.candidate.v14.lock").read_text(encoding="utf-8"))
    assert cand["neural_cortex_sha"] != v14["neural_cortex_sha"]
    mact_p = REPO_ROOT / "docs" / "cortex_mact_boundary.v15.lock"
    if mact_p.exists():
        mact = json.loads(mact_p.read_text(encoding="utf-8"))
        required = {"C4_consequence_swap_timed", "C5_plasticity_necessity", "C6_no_consequence_population"}
        if mact.get("all_required_green"):
            assert required.issubset({c["id"] for c in mact["controls"] if c.get("ok")})
    assert not (REPO_ROOT / "docs" / "cortex_development.v15.lock").exists()
    prereg = json.loads((REPO_ROOT / "docs" / "cortex_d3_r2.prereg.lock").read_text(encoding="utf-8"))
    r1p = json.loads((REPO_ROOT / "docs" / "cortex_d3_r1.prereg.lock").read_text(encoding="utf-8"))
    assert prereg["eval_seed_commitment"] != r1p["eval_seed_commitment"]
    assert prereg["authorized_law"] == "equal_evidence_min_3_symbols"
...
def test_verify_d3_r2_cli() -> None:
    ...
    v = verify_d3_r2()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v15.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d3_r2_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d3_r2_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d3_r2_gate.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v15.lock").exists()


def test_verify_d7_r2_cli() -> None:
    from experiments.run_tm023cortex import verify_d7_r2

    v = verify_d7_r2()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v25.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d7_r2_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d7_r2_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d7_r2_gate.failure.lock").exists()


def test_verify_d7_r1_cli() -> None:
    from experiments.run_tm023cortex import verify_d7_r1

    v = verify_d7_r1()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v24.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d7_r1_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d7_r1_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d7_r1_gate.failure.lock").exists()


def test_verify_d6_r2_cli() -> None:
    from experiments.run_tm023cortex import verify_d6_r2

    v = verify_d6_r2()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v22.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d6_r2_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d6_r2_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d6_r2_gate.failure.lock").exists()


def test_verify_d6_r1_cli() -> None:
    from experiments.run_tm023cortex import verify_d6_r1

    v = verify_d6_r1()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v21.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d6_r1_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d6_r1_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
...
def test_verify_d5_r2_cli() -> None:
    ...
    v = verify_d5_r2()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v20.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d5_r2_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d5_r2_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d5_r2_gate.failure.lock").exists()


def test_verify_d5_r1_cli() -> None:
    from experiments.run_tm023cortex import verify_d5_r1

    v = verify_d5_r1()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v19.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d5_r1_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d5_r1_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d5_r1_gate.failure.lock").exists()


def test_verify_d4_r1_cli() -> None:
    from experiments.run_tm023cortex import verify_d4_r1

    v = verify_d4_r1()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex.candidate.v17.lock").exists() or not (
        REPO_ROOT / "docs" / "cortex_d4_r1_gate.lock"
    ).exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["relation_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_d4_r1_gate.failure.lock").exists()
    else:
        assert v["refuse_fulldev_before_clear"] is True
        assert (REPO_ROOT / "docs" / "cortex_d4_r1_gate.failure.lock").exists()


def test_verify_fulldev_r6_cli() -> None:
    from experiments.run_tm023cortex import verify_fulldev_r6

    v = verify_fulldev_r6()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex_fulldev_r6.lock").exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["development_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_fulldev_r6.failure.lock").exists()
    else:
        assert (REPO_ROOT / "docs" / "cortex_fulldev_r6.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v13.lock").exists()


def test_verify_fulldev_r5_cli() -> None:
    from experiments.run_tm023cortex import verify_fulldev_r5

    v = verify_fulldev_r5()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex_fulldev_r5.lock").exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["development_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_fulldev_r5.failure.lock").exists()
    else:
        assert (REPO_ROOT / "docs" / "cortex_fulldev_r5.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v13.lock").exists()


def test_verify_fulldev_r4_cli() -> None:
    from experiments.run_tm023cortex import verify_fulldev_r4

    v = verify_fulldev_r4()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex_fulldev_r4.lock").exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["development_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_fulldev_r4.failure.lock").exists()
    else:
        assert (REPO_ROOT / "docs" / "cortex_fulldev_r4.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v13.lock").exists()


def test_verify_fulldev_r3_cli() -> None:
    from experiments.run_tm023cortex import verify_fulldev_r3

    v = verify_fulldev_r3()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex_fulldev_r3.lock").exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["development_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_fulldev_r3.failure.lock").exists()
    else:
        assert (REPO_ROOT / "docs" / "cortex_fulldev_r3.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v13.lock").exists()


def test_verify_fulldev_r2_cli() -> None:
    from experiments.run_tm023cortex import verify_fulldev_r2

    v = verify_fulldev_r2()
    assert v["ok"] is True, v
    if not (REPO_ROOT / "docs" / "cortex_fulldev_r2.lock").exists():
        assert v.get("pending") is True
        return
    assert v.get("pending") is False
    if v["development_gate_clear"]:
        assert v["n_pair_clear"] >= 13
        assert not (REPO_ROOT / "docs" / "cortex_fulldev_r2.failure.lock").exists()
    else:
        assert (REPO_ROOT / "docs" / "cortex_fulldev_r2.failure.lock").exists()
    assert not (REPO_ROOT / "docs" / "cortex_development.v13.lock").exists()
...
if __name__ == "__main__":
    ...
    test_verify_fulldev_r6_cli()
    test_verify_fulldev_r5_cli()
    test_verify_fulldev_r4_cli()
    test_verify_fulldev_r3_cli()
    test_verify_fulldev_r2_cli()
