"""TM.0.6.5: concurrent bind — stamp the page in play, block extra hapax here."""

from __future__ import annotations

import inspect
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPO_ROOT))

from experiments.run_tm054 import _rare_words
from experiments.run_tm054 import make as make054
from experiments.run_tm060 import make as make060
from experiments.run_tm061 import make as make061
from experiments.run_tm062 import make as make062
from experiments.run_tm063 import make as make063
from experiments.run_tm064 import make as make064
from experiments.run_tm064 import wiki_prose
from experiments.run_tm065 import make
from three_memory import agent as agent_mod
from three_memory.dial_env import STATION_NAMES, ChannelDialWorld, DialAction
from three_memory.policy import UsePolicy
from three_memory.tag_store import write_prose_notes

_MOTOR = {a.name.lower() for a in DialAction}
_STATIONS = set(STATION_NAMES.values())


def test_wiki_still_multi_rare():
    rare = _rare_words(wiki_prose(include_a=True, include_c=True))
    clutter_rare = [n for n, ws in rare.items() if n.startswith("c") and ws]
    assert len(clutter_rare) >= 3
    assert set(rare["p99.md"]) >= {"push", "argon"}


def test_in_hand_stamps_attended_page_not_leftover(tmp_path: Path):
    w = tmp_path / "W"
    s = tmp_path / "S"
    write_prose_notes(w, wiki_prose(include_a=True))
    a = make(s, w, UsePolicy(seed=7), force_write=True, explore_epsilon=0.0)
    by_id = {r.fact_id: r for r in a.world.records()}
    assert "c08" in by_id and "p99" in by_id
    a._commit_w_record(by_id["c08"])
    a._commit_w_record(by_id["p99"])
    a._in_hand_id = "p99"
    obs = ChannelDialWorld(seed=3).reset("probe_channel_a")
    a._maybe_annotate("press", obs)
    binds = [str(r.tags.get("bind") or "").lower() for r in a.store.records()]
    assert "push" in binds
    assert "xenon" not in binds


def test_block_here_does_not_add_a_second_cs(tmp_path: Path):
    w = tmp_path / "W"
    s = tmp_path / "S"
    write_prose_notes(w, wiki_prose(include_a=True))
    a = make(s, w, UsePolicy(seed=7), force_write=True, explore_epsilon=0.0)
    by_id = {r.fact_id: r for r in a.world.records()}
    a._commit_w_record(by_id["p99"])
    a._in_hand_id = "p99"
    obs = ChannelDialWorld(seed=3).reset("probe_channel_a")
    a._maybe_annotate("press", obs)
    a._commit_w_record(by_id["c08"])
    a._in_hand_id = "c08"
    a._maybe_annotate("press", obs)
    binds = [str(r.tags.get("bind") or "").lower() for r in a.store.records() if r.tags.get("bind")]
    assert binds == ["push"]
    assert "xenon" not in binds


def test_untrained_holds(tmp_path: Path):
    import numpy as np
    from experiments.run_tm052 import live_free

    w = tmp_path / "W"
    s = tmp_path / "S"
    write_prose_notes(w, wiki_prose(include_a=True))
    a = make(s, w, UsePolicy(seed=7), explore_epsilon=1.0, rng=np.random.default_rng(0))
    live = live_free(a, "experience_channel_a", 3, max_steps=32)
    assert live["n_annotated"] == 0
    a.world = None
    a.reset_rho()
    act_a, _ = a.act(ChannelDialWorld(seed=3).reset("probe_channel_a"), update_rho=False, explore=False)
    a.reset_rho()
    act_c, _ = a.act(ChannelDialWorld(seed=3).reset("probe_channel_c"), update_rho=False, explore=False)
    assert act_a == int(DialAction.HOLD)
    assert act_c == int(DialAction.HOLD)


def test_skips_stay_skipped():
    src_agent = Path(REPO_ROOT / "three_memory" / "agent.py").read_text(encoding="utf-8")
    src_exp = Path(REPO_ROOT / "experiments" / "run_tm065.py").read_text(encoding="utf-8")
    assert "has_code" in src_agent
    assert "use_block_here" in src_agent
    assert 'domain="dial"' in src_exp
    assert "KeyDoorWorld" not in src_exp
    a = agent_mod.ThreeMemoryAgent(use_policy=UsePolicy(seed=1), store_enabled=False, cortex_seed=1337)
    assert a.domain == "door" and not a.use_block_here and not a.use_stamp_new_here
    b = make054(Path("/tmp/tm065_skip_s"), None, UsePolicy(seed=1), enabled=False)
    assert not b.use_block_here
    c = make060(Path("/tmp/tm065_skip_060"), None, UsePolicy(seed=1), enabled=False)
    assert not c.use_block_here
    d = make061(Path("/tmp/tm065_skip_061"), None, UsePolicy(seed=1), enabled=False)
    assert d.use_one_bind and not d.use_block_here
    e = make062(Path("/tmp/tm065_skip_062"), None, UsePolicy(seed=1), enabled=False)
    assert not e.use_block_here and not e.use_stamp_new_here
    f = make063(Path("/tmp/tm065_skip_063"), None, UsePolicy(seed=1), enabled=False)
    assert f.use_stamp_new_here and not f.use_block_here
    g = make064(Path("/tmp/tm065_skip_064"), None, UsePolicy(seed=1), enabled=False)
    assert g.use_stamp_new_here and not g.use_block_here
    h = make(Path("/tmp/tm065_skip_065"), None, UsePolicy(seed=1), enabled=False)
    assert h.use_block_here and h.use_stamp_new_here and h.use_one_bind
    copy = make(Path("/tmp/tm065_copy"), None, UsePolicy(seed=1), enabled=False, use_here_match=False)
    assert not copy.use_block_here


def test_agent_source_no_synonym_table():
    src = inspect.getsource(agent_mod)
    assert '"push"' not in src and "'push'" not in src
    assert '"xenon"' not in src and "'xenon'" not in src
    assert '"krypton"' not in src and "'krypton'" not in src


if __name__ == "__main__":
    import tempfile

    test_wiki_still_multi_rare()
    test_skips_stay_skipped()
    test_agent_source_no_synonym_table()
    fns = [test_in_hand_stamps_attended_page_not_leftover, test_block_here_does_not_add_a_second_cs, test_untrained_holds]
    for fn in fns:
        with tempfile.TemporaryDirectory() as d:
            fn(Path(d))
    print("ok")
