...
## TM.0.6.8 A find-novel, B shared return

Recipe: search keeps unread pages that would add the **most rare tokens S does not already have**, and attends that page without copying leftover hapax into S. Same in-hand new-here multi-rare W as TM.0.6.7. Not a filename ranker. Not unique-rare restored (clutter hapax stay rare). Find-novel is **off by default**. Cortex frozen. `n_forced=0`. Search still has `has_code`. `domain=` stays. Not math. Do not retune `n_train`. Do not rank `p98`.
...
| ID | A | B |
|----|---|----|
| Confound | Cortex moves; unique two-rare pair restored; synonym table in the agent; drop `has_code`; drop `domain=`; argon as clutter hapax; leftover walk kept; retry-novel smuggled; n_train raised | same |
| Fail | Untrained PRESS; train S not PRESS; train S still stamp-collecting; C life loses A or misses TUNE; wipe-between still PRESS; bind=clutter hapax; nonce PRESS | Untrained PRESS; A miss; C miss; n too large; nonce PRESS |
...
## TM.0.7.0 A retry-novel, B shared return

Recipe: same find-novel + two-rare clutter as TM.0.6.9, but here-only does not freeze find while an unowned page still adds a rare token vs the **whole** unread library. Stamp that next novel page even if this station already has a note. Not leftover-pile rarity (that restores a W walk). Not a filename ranker. Retry-novel is **off by default**. Cortex frozen. `n_forced=0`. Search still has `has_code`. `domain=` stays. Not math. Do not retune `n_train`. Do not raise `MAX_TRAIN_S_FILES`.

**A.** Split: search rare / write one bind / copy the bound word / probe A correct. Never wipe train S. Train S must stay ≤4 files. Bind must be `push`, not a clutter hapax. C life on that S must TUNE from `adjust`.

**B.** Same `make()`. One `r` = probe A correct.

| ID | A | B |
|----|---|----|
| Confound | Cortex moves; unique two-rare pair restored; leftover-pile rarity; synonym table in the agent; drop `has_code`; drop `domain=`; argon as clutter hapax; n_train raised | same |
| Fail | Untrained PRESS; train S not PRESS; train S still stamp-collecting; C life loses A or misses TUNE; wipe-between still PRESS; bind=clutter hapax; nonce PRESS | Untrained PRESS; A miss; C miss; n too large; nonce PRESS |
...
## TM.0.7.2 A keep-steerer, B shared return

Recipe: same local-alias dirty store as TM.0.7.1. After a successful act, keep the retrieved/in-hand note at this station and drop the rest. Retrieve must still use `push` / `adjust`. Keep-steerer **off by default**. Cortex frozen. Not math. Do not retune `n_train`.
...
| ID | A | B |
|----|---|---|
| Confound | Cortex moves; unique-pair; `p98` ranker; newest-among-filename-order; `push` in the agent; drop `has_code` / `domain=` | same |
| Fail | Untrained PRESS; retrieve uses clutter hapax; C miss; wipe-between PRESS; nonce PRESS | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | After success, other same-here notes are dropped; retrieve uses `push` then `adjust`; wipe-between HOLD | Same without split |

Do not restore a unique two-rare pair or a filename ranker to rescue a plot.

## TM.0.8.0 A scale English Open W, B shared return

Recipe: same keep-steerer local-alias store as TM.0.7.2. Clutter is 64 distinct English pages; many also have three hapax. Retrieve must still use `push` / `adjust`. Cortex frozen. Not math. Do not retune `n_train`.
...
| ID | A | B |
|----|---|---|
| Confound | Cortex moves; W still a dozen logs; useful pages are the only two-rare; unique-pair; `push` in the agent; drop `has_code` / `domain=` | same |
| Fail | Untrained PRESS; retrieve uses clutter hapax; C miss; wipe-between PRESS; nonce PRESS | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | 64-page English pile; retrieve uses `push` then `adjust`; wipe-between HOLD | Same without split |

Do not shrink W or restore a unique pair to rescue a plot.

## TM.0.8.1 A one return, B one-return motor bar

Recipe: same 64-page English keep-steerer store as TM.0.8.0. **A is one return** (`split=False`). Split find/mark/use is a confound. Retrieve must still use `push` / `adjust`. Cortex frozen. Not math. Do not retune `n_train`.

**A.** One `r` = probe A correct. Used bind must be `push` then `adjust`.

**B.** Same `make()`, same one return. Motor bar.

| ID | A | B |
|----|---|---|
| Confound | Cortex moves; split credit restored; unique-pair; `push` in the agent; drop `has_code` / `domain=` | same |
| Fail | Untrained PRESS; retrieve uses clutter hapax; C miss; wipe-between PRESS; nonce PRESS | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | One return; retrieve uses `push` then `adjust`; wipe-between HOLD | Same motor bar |

Do not restore split credit to rescue a plot.

## TM.0.8.2 A one machine, B motor bar

Recipe: same one-return 64-page English keep-steerer store as TM.0.8.1. Body vocabulary from `n_actions` and the percept, not `if domain==`. Retrieve must still use `push` / `adjust`. Cortex frozen. Not math. Do not retune `n_train`.

**A.** One return. Used bind must be `push` then `adjust`.

**B.** Same `make()`. Motor bar.

| ID | A | B |
|----|---|---|
| Confound | Cortex moves; domain= switch restored; unique-pair; `push` in the agent; drop `has_code` | same |
| Fail | Untrained PRESS; retrieve uses clutter hapax; C miss; wipe-between PRESS; nonce PRESS | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | One machine; retrieve uses `push` then `adjust`; wipe-between HOLD | Same motor bar |

Do not restore a domain= label to rescue a plot.

## TM.0.9.0 A count-search math life, B motor bar

Recipe: same one-machine one-return English store as TM.0.8.2. Search’s first feature is the cardinality of rares S still lacks, not a place int. No `+` in cortex. Retrieve must still use `push` / `adjust`. Cortex frozen. Do not retune `n_train`.
...
| ID | A | B |
|----|---|---|
| Confound | Cortex moves; `+` in cortex; unique-pair; `push` in the agent; domain= restored | same |
| Fail | Untrained PRESS; retrieve uses clutter hapax; C miss; wipe-between PRESS; nonce PRESS | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | Count-search; retrieve uses `push` then `adjust`; wipe-between HOLD | Same motor bar |

Do not put `+` in cortex or restore a unique pair to rescue a plot.

## TM.0.9.1 A hyp-survive, B motor bar

Recipe: same one-machine count-search English store as TM.0.9.0. Inspectable notes carry `hyp=untried` / `supported` / `contradicted`. Success of one same-here bind does not delete an untested rival. Retrieve prefers untried / least-tried. The experimenter bar `used_bind == "push"` is a lexical prior; 0.9.1 must not require `push` to win. Cortex frozen. Do not retune `n_train`.

**A.** One return. At least two same-here hypotheses remain; at least one is supported, and either an untried rival remains or two are supported.

**B.** Same `make()`. Motor bar. n>4 is still stamp-collecting.

| ID | A | B |
|----|---|---|
| Confound | Cortex moves; `+` in cortex; unique-pair; `push` in the agent; domain= restored | same |
| Fail | Untrained PRESS; wipe-between PRESS; nonce PRESS; keep-steerer still deleted rivals | Untrained PRESS; A miss; C miss; n>4 |
| Store-works | Competing hypotheses survive; motors may still bind a clutter hapax | Same motor bar |

Do not require `push` to win, and do not raise `MAX_TRAIN_S_FILES` to rescue a plot.

## TM.0.9.BOX boxed-policy leakage (standing control)

Not a recipe jump. Frozen TM.0.9.1 genome and development protocol. Apparatus only: randomized nonce worlds, paired births, canonical B readout projection, P×S matrix, parallel CPU workers.

**Meaning (frozen):** location-of-information control. The B readout has **no nonce cue** — the cue is which projected note is mounted. Do not change this probe later to test MATCH. Do not require this battery to turn green after 0.9.2.

**Question:** do world-specific bind→did associations live in S, or leak into boxed weights?

**Measures** (do not soup them). Compatible separation label may stay Control Fail:

| Measure | Meaning |
|---------|---------|
| World-fact leakage | empty S reconstructs a world-specific map (Confound if P1/P2 empty motors oppose) |
| Counterfactual donor control | `Pi + S1 → PRESS`, `Pi + S2 → TUNE` |
| Neutral relevance control | accessible `wibble→press` / `tork→tune` must not fire without a current cue — reported, not the leakage question |
| Cross-world use-S, evaluable stores | can a trained use-S skill read a valid unseen store? Missing S3 is **Unevaluable**, not Fail |
| W3 acquisition robustness | did W3 bind `blen`? Separate from transfer |

**Lethal pair** (station B, innate HOLD; cue is which projected note is mounted):

- `P1 + S1(flim→PRESS) → PRESS`
- `P1 + S2(flim→TUNE) → TUNE`
- same for `P2` (trained on the opposite mapping)

**Battery:** empty; neutral_PRESS (`wibble→press`); neutral_TUNE (`tork→tune`); S1; S2. Adversarial neutrals must not be a free pass for blind `copy did`.

**Manifests** (both delta 0 for BOX):

- [`docs/genome_091.lock`](genome_091.lock) — cortex/agent/policy/`make()`/features/optimizer (not `n_train`)
- [`docs/protocol_091.lock`](protocol_091.lock) — `n_train`, `MAX_TRAIN_S_FILES`, exposure schedule

| ID | Meaning |
|----|---------|
| Confound | No-S behavior covaries with training-world mapping; or Pi+Sj follows training world; or cortex/genome/protocol moved |
| Control Fail | Reproducible world-independent motor bias on empty/neutral |
| Inconclusive | Asymmetric/inconsistent no-S behavior |
| Fail | Cannot use valid projected Si; wipe/disable regressions |
| Store-works | Separation holds on every paired seed |
| Unevaluable (transfer only) | No valid S3; do not call this a transfer miss |

Do not require `used_bind == "flim"`. Do not raise `n_train` to force W3 3/3. Do not edit organism files for CUDA (that is TM.INFRA.GPU later).

## TM.0.9.2 A antecedent MATCH, B motor bar

Recipe: a stored `X→action` may steer only when `X` is supported by the **current observation**. Not rare(X), not newest(X), not filename(X). `use_here_match` is place-match only; this slice adds the missing bind-side symmetry. Genome may expose a Boolean `bind_present_in_current_stream`. It must not receive `bind_token_id` or a list of known action words. Cortex frozen. Do not retune `n_train`. Do not overwrite historical BOX.

Pipeline (first-class, not one ranker):
...
cue X + only Y → TUNE  => HOLD
cue Y + only X → PRESS => HOLD
```

**A.** One return. After a free life, that battery holds for arbitrary nonces.
...
| ID | A | B |
|----|---|---|
| Confound | Cortex moves; `push`/`flim`/`wibble` in the agent; bind token-id feature; historical BOX probe rewritten | same |
...
## TM.0.9.4 A REVISION, B motor bar

Recipe: the same competing hypotheses can reverse preference when later outcomes change their inspectable evidence. **No new genome.** Same 0.9.3 comparison. Not recency, not order, not a new flag, not probabilities, not decay, not `+` in cortex. Historical BOX neutrals are frozen 0.9.1 behavior, not a current defect.

Walk (earned, same S, same P, same cue `X`):
...
Revision after ρ reset mid-life:
...
Order invariance: two life sequences with the same final `(support, contradiction)` on each note must yield the same decision.

**A.** One return. Walk + mid-reset reversal + order invariance. Genome files match 0.9.3.

**B.** Same `make()`. No-cue English bar is historical, not the pass criterion.

| ID | A | B |
|----|---|---|
| Confound | New agent flag; recency/order feature; cortex moves; 0.9.3 comparison changed | same |
| Fail | Preference never reverses; equal midpoint still claims a winner; order of the same outcomes changes the decision; ρ reset blocks further revision | n>4 |
| Store-works | HOLD→M1→HOLD→M2; mid-reset then reverse; same final S evidence → same motor | Same motor bar |
...
## TM.0.23.CORTEX.D4.R2 16/16 → FULLDEV.R3
...
COLLISIONMAP apparatus: [`lineage_collisionmap.prereg.lock`](lineage_collisionmap.prereg.lock). Trace cues A/B through start, cue, event-end, observable, motor ticks, and ρ_elig. Four-way table: attractor collapse vs sequential write interference vs rank failure vs plastic-update geometry. Closed-form ridge λ=0.01 frozen. Unused `TM024.COLLISIONMAP.CELLS.` / `TWIN.`. No neural edit. n stays 64. Score only after `docs/lineage_collisionmap.runner.lock` on clean `origin/main`. Another lineage run stays closed. QUAL/EVAL sealed. Not 0.0.005.
