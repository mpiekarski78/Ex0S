...
- TM.0.9.BOX: boxed-policy leakage control on frozen 0.9.1 genome (delta 0). Separation **Control Fail** (empty HOLD; counterfactual S follows donor; neutrals blind-copy `did`). Transfer **Fail** (2/3 seeds Pass). Not Confound — facts did not leak into P as a world-correlated map. [`docs/tm091box_results.md`](tm091box_results.md).
